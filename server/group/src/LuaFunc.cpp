...
BOOL GetOnlineCount(DWORD& dwLoginNum, DWORD& dwPlayerNum) {
	...
	// Initialize the NPC script global variable information
	lua_getglobal(g_pLuaState, "apicall_get_online_count");
	if (!lua_isfunction(g_pLuaState, -1)) {
		lua_pop(g_pLuaState, 1);
		LG("lua_invalidfunc", "apicall_get_online_count");
		return FALSE;
	}
	...
	if (nStatus) {
		printf("call [apicall_get_online_count] failed!\n");
...
inline int lua_GetResString(lua_State *L) {
	T_B
		...
		char* text = const_cast<char*>(CResourceBundleManage::Instance()->LoadResString(pszID));
