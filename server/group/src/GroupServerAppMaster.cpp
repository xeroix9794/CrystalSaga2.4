	...
	if (bInvited) {
		...
		if (l_inviter && l_inviter->m_currcha >= 0 && l_inviter.m_chaid == l_inviter->m_chaid[l_inviter->m_currcha]) {
			...
			if ((++(pPrentice->m_CurrMasterNum)) > const_master.MasterMax) {
				...
				l_wpk.WriteString(Master::GetMasterGroup());
				...
				l_wpk2.WriteString(Master::GetPrenriceGroup());
...
void GroupServerApp::MP_MASTER_FINISH(Player *ply, DataSocket *datasock, RPacket &pk) {
	...
	// MRF_WHY: This is weird, the condition and the body seem to use 2 different approaches...
...
void Player::MasterInvitedCheck(Invited	*invited) {
	...
	// MRF_WHY: Why was this commented out? maybe because a prenticeMax was no longer required?
	/*
...
void GroupServerApp::PC_MASTER_INIT(Player *ply) {
	...
	for (int i = 0; i < l_num1; i++) {
		...
		else if ((l_ply11 = (Player*)MakePointer(l_farray1[i].memaddr)) && ((l_currcha1 = l_ply11->m_currcha) >= 0) && (l_ply11->m_chaid[l_currcha1] == l_farray1[i].cha_id)) {
	...
	for (int i = 0; i < l_num2; i++) {
		...
		else if ((l_ply12 = (Player*)MakePointer(l_farray2[i].memaddr)) && ((l_currcha2 = l_ply12->m_currcha) >= 0) && (l_ply12->m_chaid[l_currcha2] == l_farray2[i].cha_id)) {
