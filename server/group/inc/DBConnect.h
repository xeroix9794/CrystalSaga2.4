...
public:
	...
	char GetGM() {
		return atoi(m_buf[1].c_str());
	}

	const char *GetChaIDs() {
		return m_buf[2].c_str();
	}

	const char *GetPassword() {
		return m_buf[3].c_str();
	}

	const char *GetLast_IP() {
		return m_buf[4].c_str();
	}

	const char *GetDisc_Reason() {
		return m_buf[5].c_str();
	}

	const char *GetLast_Leave()	{
		return m_buf[6].c_str();
	}
...
public:
	...
	const char *GetMottonByName() {
		return m_buf[1].c_str();
	}

	short GetIconByName() {
		return atoi(m_buf[2].c_str());
	}
	...
	short GetDegree() {
		return atoi(m_buf[6].c_str());
	}

	const char *GetMap() {
		return m_buf[7].c_str();
	}

	int GetMap_X() {
		return atoi(m_buf[8].c_str());
	}

	int GetMap_Y() {
		return atoi(m_buf[9].c_str());
	}

	const char *GetLook() {
		return m_buf[10].c_str();
	}

	int GetStr() {
		return atoi(m_buf[11].c_str());
	}

	int GetDex() {
		return atoi(m_buf[12].c_str());
	}

	int GetAgi() {
		return atoi(m_buf[13].c_str());
	}

	int GetCon() {
		return atoi(m_buf[14].c_str());
	}

	int GetSta() {
		return atoi(m_buf[15].c_str());
	}

	int GetLuk() {
		return atoi(m_buf[16].c_str());
	}
...
public:
	struct master_dat {
		...
		string cha_name;		// Character Name
...
public:
	...
	int GetGuildLevel(int guildid);
	int GetGuildColor(int guildID);
	bool SaveGuildColor(int guildID, int color);
	...
	bool GetGuildMemberNum(uLong gldid, InterLockedLong& memberNum);
	int GetRowValue(int guildID,char* gParameter);
private:
