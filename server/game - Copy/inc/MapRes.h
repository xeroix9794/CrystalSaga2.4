...
enum EMapAdmin
{
	enumMAPADMIN_OPEN_MAP,			// 打开地图
	enumMAPADMIN_CLOSE_MAP,			// 关闭地图
	enumMAPADMIN_OPEN_MAP_ENTRY,	// 打开入口
	enumMAPADMIN_CLOSE_MAP_ENTRY,	// 关闭入口
	enumMAPADMIN_CHECK_MAP,			// 检查地图
};
...
public:
	...
	// Add by lark.li 20090331 begin for 地图状态检查
	void		CheckMapState(string& str);
	...
	struct
	{
		dbc::cShort		m_csEyeshotCellWidth;	// 管理单元宽度
		dbc::cShort		m_csEyeshotCellHeight;	// 管理单元高度
	...
	struct
	{
		CBlockData		m_CBlock;				// 地图的障碍物信息
		dbc::cShort		m_csBlockUnitWidth;		// 障碍物宽度单位（缺省50）
		dbc::cShort		m_csBlockUnitHeight;	// 障碍物高度单位（缺省50）
	};
	...
	struct
	{
		...
		// Add by lark.li 20081225 begin
		string      m_strEntryFile;			// 入口文件脚本
		// End
	};
