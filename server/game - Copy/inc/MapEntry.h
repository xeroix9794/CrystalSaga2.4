...
public:
	...
	void			SetMapName(dbc::cChar *cszMapName) {if (!cszMapName) return; /*strncpy(m_szMapName, cszMapName, MAX_MAPNAME_LENGTH - 1);*/strncpy_s( m_szMapName, sizeof(m_szMapName), cszMapName, _TRUNCATE ); m_szMapName[MAX_MAPNAME_LENGTH -1] = '\0';}
	dbc::cChar*		GetMapName(void) const {return m_szMapName;}
	void			SetTMapName(dbc::cChar *cszTMapName) {if (!cszTMapName) return;/* strncpy(m_szTMapName, cszTMapName, MAX_MAPNAME_LENGTH - 1);*/strncpy_s( m_szTMapName, sizeof(m_szTMapName), cszTMapName, _TRUNCATE ); m_szTMapName[MAX_MAPNAME_LENGTH -1] = '\0';}
