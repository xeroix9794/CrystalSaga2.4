...
{T_B
	...
	strncpy_s( m_name, sizeof(m_name), READ_STRING(rpk), _TRUNCATE );

	Char szLogName[1024];
	//strcpy(szLogName, READ_STRING(rpk));
	strncpy_s( szLogName, sizeof(szLogName), READ_STRING(rpk), _TRUNCATE );
...
{T_B
	...
	if (!pCMap)
	{
		...
		LG("eyeshot activation error", "when entity %s is doing eye shot notify,the map is null\n", GetLogName());
		return;
	}
			...
			for(x =l_rect.ltop.x;x <=l_rect.rbtm.x;x++)
			{
				...
				while (pCTarEnt)
				{
					if (++lEntCount > lEntNum)
					{
						...
						LG("eyeshot activation error", "eye shot cell [%d,%d] the fact entity number %d\n", x, y, lEntNum);
						break;
					}
					...
					if(pCTarCha->IsPlayerFocusCha())
					{
						if ((pCSrcCha && pCSrcCha->CanSeen(pCTarCha)) || !pCSrcCha)
						{
							...
							if (!pCTarCha->GetSubMap())
								...
								LG("eyeshot activation error", "when entity %s[%d,%d] is doing eye shot notify, the aim player %s(%s)[%d,%d] map is null\n",
	...
	catch (...)
	{
		...
		LG("eyeshot activation error", "entity %s eye shot notify error(coordnate [%d,%d],eye shot [%d,%d;%d,%d]),currently eye shot cell[%d,%d]\n", cszSrcLogName,
			l_pos1.x, l_pos1.y, l_rect.ltop.x, l_rect.ltop.y, l_rect.rbtm.x, l_rect.rbtm.y, x, y);
		throw;
	}
