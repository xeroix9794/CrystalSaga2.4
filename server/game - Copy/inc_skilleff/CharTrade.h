...
#pragma pack( 8 )
    ...
    public:
		...
		// 购买道具退款
		BOOL AcceptReturn( long long lOrderID, RoleInfo *ChaInfo );
...
#ifdef SHUI_JING
		//角色水晶号查询
		BOOL RequestActInfo( CCharacter* pCha );
		BOOL AcceptActInfo( long long lOrderID, RoleInfo* ChaInfo );
		BOOL CancelActInfo( long long lOrderID );

		//水晶具体兑换
		BOOL RequestCrystalAdd( CCharacter* pCha, string act_tName, DWORD cha_tid, DWORD dwrplMoney, DWORD iCslPrice, DWORD iCslNum, DWORD dcslnum, int type, char quicktype, DWORD m_TAX );
		BOOL AcceptCrystalAdd( long long lOrderID, CrystalInfo* cryInfo );
		BOOL CancelCrystalAdd( long long lOrderID, CrystalInfo* cryInfo );

		BOOL RequestCrystalDel( CCharacter* pCha, string act_tName, DWORD cha_tid, DWORD dwrplMoney, DWORD iCslPrice, DWORD iCslNum, DWORD dcslnum, int type, char quicktype, DWORD m_TAX );
		BOOL AcceptCrystalDel( long long lOrderID, CrystalInfo* cryInfo );
		BOOL CancelCrystalDel( long long lOrderID, CrystalInfo* cryInfo );
		//定时更新状态
		BOOL RequestCrystalAdd( string act_Name, DWORD cha_id, string cha_name, DWORD dwrplMoney, DWORD iCslPrice, DWORD iCslNum, DWORD dcslnum, DWORD m_TAX  );
		//返还水晶后操作
		BOOL AcceptCrystalAddReturn( long long lOrderID, CrystalInfo* cryInfo );
		BOOL CancelCrystalAddReturn( long long lOrderID, CrystalInfo* cryInfo );
		BOOL AcceptCrystalDelReturn( long long lOrderID, CrystalInfo* cryInfo );
		BOOL CancelCrystalDelReturn( long long lOrderID, CrystalInfo* cryInfo );
#endif
		//用户开启商城
		...
		//水晶平台开启状态	
		void SetCrystalTradeOpen(bool val)  { m_bTrade = val;  }
		BOOL IsCrystalTrade() { return m_bTrade; }
    ...
    private:
		...
		BOOL		PushOrder( DWORD cha_id, string cha_name, long long lOrderID, long lComID, long lNum );//Add by sunny.sun 20090519
