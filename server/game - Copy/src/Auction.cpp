...
BOOL CAuctionSystem::StartAuction(short sItemID, const string& strName, short sCount, uInt nBasePrice, uInt nMinBid)
{
	...
	if(it != m_mapItemList.end())
	{
		...
		param.setDouble( 0, sItemID );
		param.setString( 1, strName.c_str() );
		RES_FORMAT_STRING( GM_AUCTION_CPP_00001, param, szData );
	...
	if(pAucItem == NULL)
	{
		...
		param.setDouble( 0, sItemID );
		param.setString( 1, strName.c_str() );
		RES_FORMAT_STRING( GM_AUCTION_CPP_00002, param, szData );
