...
void Guild::cmd_CreateGuild(CCharacter* pCha, bool confirm, cChar *guildname, cChar *passwd)
{
	...
	strncpy_s( l_guildname, sizeof(l_guildname), guildname, _TRUNCATE );
	long l_guildid =game_db.CreateGuild(pCha,pCha->GetPlayer()->m_cGuildType,l_guildname,passwd);
	if(!l_guildid)
	{
		return;
	}
...
{
	pCha->GetPlayer()->m_cGuildType	=guildtype;
	game_db.ListAllGuild(pCha,guildtype,7);
}
void Guild::cmd_GuildTryFor(CCharacter* pCha, uLong guildid)			//申请加入公会
{
	...
	{
		char	l_gldtype	=game_db.GetGuildTypeByID(pCha,guildid);
...
void Guild::cmd_GuildCancelTryFor( CCharacter* pCha )
{
	if( pCha )
	{
		game_db.GuildCancelTryFor( pCha );
	}
}