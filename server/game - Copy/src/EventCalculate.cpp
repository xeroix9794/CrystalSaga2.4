...
void CEventHandler::Event_ChaDie(CCharacter *pDead, CCharacter *pAtk)
{
	...
	if(r!=0) // 执行出错
	{
		...
		LG("lua_err", "GetExp_New do error, attacker[%s], monster was bring down [%s]!\n", pAtk->GetName(), pDead->GetName());
		lua_callalert(g_pLuaState, r); 	
	}
	lua_settop(g_pLuaState, 0);
	tLua.End();
	...
	LETimer tMission; tMission.Begin();
	// 攻击者和他的队友得到任务通知
