...
//#define PARAM_ERROR        { LG("lua_ai", "lua扩展函数[%s]参数个数或者类型错误!\n",__FUNCTION__ ); }
//#define MAP_NULL_ERROR     { LG("lua_ai", "lua扩展函数[%s]当前地图为空\n", __FUNCTION__);          }
...
#define REGFN(fn)  { lua_pushstring(L, "" #fn ""); \
	...
	if(find(g_luaFNList.begin(), g_luaFNList.end(), ""#fn"")!=g_luaFNList.end()) LG("lua", "msgind register lua the same functing[%s]\n", ""#fn""); else \
	  g_luaFNList.push_back(""#fn"");  } 
    // if(find(g_luaFNList.begin(), g_luaFNList.end(), ""#fn"")!=g_luaFNList.end()) LG("lua", "msg 发现登记lua同名函数[%s]\n", ""#fn""); else \
	// g_luaFNList.push_back(""#fn"");  }
...
struct SHelpNPC
{
	...
	{
		//strcpy(szName, "");
		strncpy_s(szName,sizeof(szName), "",_TRUNCATE);
		pNPC = NULL;
	}
};

extern std::list<CCharacter*> g_HelpNPCList;
extern mission::CNpc* pTradeNPC;
...
{T_B
 	lua_dostring(g_pLuaState, "RunTimer()");
T_E}
...
inline void lua_AIRun(CCharacter *pCha)
{T_B
	
	static int g_test[20];
	lua_getglobal(g_pLuaState, "ai_loop");
...
{T_B
	...
	for( int i = 3; i <= nNumParam; i++ )
	{
		...
		if( sNum > 1020 )/* strcat( szData, "..." )*/strncat_s( szData, sizeof(szData), "...", _TRUNCATE);
		str << szData;
		switch( *(pszPos + 1) )
		{
		case 'd':
...
//通过角色ID获得角色指针
//Add by sunny.sun 20081031
//Begin
inline int lua_GetRolebyID(lua_State * L)
{
	BOOL bVaild = (lua_gettop (L) == 1 && lua_isnumber(L, 1));
	if(!bVaild)
	{
		PARAM_ERROR
		return  0;
	}
	int iplayID = (int)lua_tonumber(L, 1);
	if( iplayID <= 0 )
	{
		E_LUANULL
		return 0;
	}
	CPlayer*	player = g_pGameApp->GetPlayerByDBID(iplayID);
	CCharacter * pCha = NULL;
	if(player)
	{
		pCha = player->GetMainCha();
	}
...
inline int lua_GetRolebyName(lua_State * L)
{
	BOOL bVaild = (lua_gettop(L) == 1 && lua_isstring(L, 1));
	if (!bVaild)
	{
		PARAM_ERROR
			return  0;
	}
	const char* cCharName = lua_tostring(L, 1);
	CPlayer* player = g_pGameApp->GetPlayerByMainChaName(cCharName);
...
inline	int	lua_UnlockItem(	lua_State*	L)
{
	...
	if(	pPlayer	)
	{
		int		iCapacity	=	pPlayer->GetMainCha()->GetKitbag()->GetCapacity();

		for(	int	i	=	0;	i	<	iCapacity;	i	++	)
		{
			SItemGrid*	sig	=	pPlayer->GetMainCha()->GetKitbag()->GetGridContByID(	i	);
