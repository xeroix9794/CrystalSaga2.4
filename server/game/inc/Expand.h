...
inline int lua_MSG_TEST(lua_State *pLS)
{T_B
	const char* pszMsg = lua_tostring( pLS, 1);

	extern void SendScriptMsg(const char* msg);
	SendScriptMsg(pszMsg);
...
#ifdef defPARSE_LOG
//g_pCLogObj->Log("获取角色属性 GetChaAttr\n");
g_pCLogObj->Log("Get character attribute : GetChaAttr\n");
#endif
...
{
#ifdef defPARSE_LOG
	//g_pCLogObj->Log("\t无此属性：[%d]，调用失败\n", sAttrIndex);
	g_pCLogObj->Log("\tHaven't this attribute :[%d],transfer failed \n", sAttrIndex);
#endif
	bSuccess = false;
	goto End;
}

if(sAttrIndex == ATTR_CEXP || sAttrIndex == ATTR_NLEXP || sAttrIndex == ATTR_CLEXP)
...
#pragma warning(disable: 4244)
	...
	else
	{
		if (nAttrVal < 0 && g_IsNosignChaAttr(sAttrIndex))
			// 这边经验值应该是不会走到的 drog
			lua_pushnumber(pLS, (unsigned)nAttrVal);
		else
			lua_pushnumber(pLS, nAttrVal);
	}
...
#ifdef defPARSE_LOG
//g_pCLogObj->Log("设置角色属性 SetChaAttr\n");
g_pCLogObj->Log("Setting character attribute : SetChaAttr\n ");
#endif
...
{
#ifdef defPARSE_LOG
	//g_pCLogObj->Log("设置属性失败，无此属性 %d", sAttrIndex);
	g_pCLogObj->Log("Setting attribute failed,haven't this attribute %d", sAttrIndex);
#endif
	bSuccess = false;
	goto End;
}
...
//		_snprintf_s(szGene + strlen(szGene),sizeof(szGene + strlen(szGene)),_TRUNCATE, "%d,", sRangeE[i]);
...
if (!pCCha->LearnSkill((short)nSkillID, (char)nSkillLv, bSetLv, bUsePoint))
{
#ifdef defPARSE_LOG
...
//Add by sunny.sun 20081105 曾亚要求获取技能的前置技能编号和等级
//参数 技能ID
//返回值 前置技能ID，等级
inline int lua_GetsPremissSkill( lua_State *pLS )
{
	CSkillRecord *pCSkill = NULL;
	int nParaNum = lua_gettop(pLS); // 取得参数个数

	if (nParaNum != 1 ||!lua_isnumber(pLS, 1) )
	{
		PARAM_ERROR
		return 0;
	}
	int nSkillID = (int)lua_tonumber( pLS, 1 );
	pCSkill = GetSkillRecordInfo( nSkillID );
	if( !pCSkill )
	{
		E_LUANULL
		return 0;
	...
	int nPremissSkillID =  pCSkill->sPremissSkill[0][0];
	int nPremissSkillLV =  pCSkill->sPremissSkill[0][1];
	if(nPremissSkillID==-1||nPremissSkillID==-2)
	{
	...
	{
		lua_pushnumber(pLS, nPremissSkillID);
		lua_pushnumber(pLS, nPremissSkillLV);
		return 2;
	}
}
//End
...
// 给任务时使用道具失败
inline int lua_UseItemNoHint(lua_State *pLS)
{T_B
	bool	bSuccess = true;
...
inline int lua_CheckBagItem(lua_State *pLS)
{
	...
	short sGridNum = pCCha->GetKitbag()->GetUseGridNum();
	SItemGrid *pGridCont;
	for (short i = 0; i < sGridNum; i++)
	{
		pGridCont = pCCha->GetKitbag()->GetGridContByNum(i);
...
inline int lua_GetChaFreeBagGridNum(lua_State *pLS)
{
	...
	sFreeNum = pCCha->GetKitbag()->GetCapacity() - pCCha->GetKitbag()->GetUseGridNum();
...
AGAIN:
	...
	sLeftNum = sNum;
	pCCha->GetKitbag()->SetChangeFlag(false);
	DelCont.sID = sID;
	for (short i = 0; i < sGridNum; i++)
	{
		pGridCont = pCCha->GetKitbag()->GetGridContByNum(i);
		...
		if( pCCha->KbPopItem(true, true, &DelCont, pCCha->GetKitbag()->GetPosIDByNum(i)) == 0 )
		{
			sLeftNum -= DelCont.sNum;
			sNum = sLeftNum;
...
inline int lua_GetCloakLevel(lua_State *pLS)
{
	short id = 9205;
	short level = 0;
	bool bSuccess = true;
	...
	pSItem = pCha->GetItem(2, id);
End:
	if (pSItem)
	{
		level = pSItem->GetInstAttr(ITEMATTR_MAXENERGY);
		lua_pushnumber(pLS, level);
		return 1;
	}
	else
		lua_pushnumber(pLS, 0);
...
inline int lua_HasCloak(lua_State *pLS)
{
	short id = 9205;
	bool bSuccess = true;
	SItemGrid *pSItem = 0;
...
inline int lua_GetCloak(lua_State *pLS)
{
	bool bSuccess = true;
	...
	SItemGrid* Cloak = (SItemGrid *)lua_touserdata(pLS, 1);
...
End:
	if (bSuccess)
	{
		lua_pushlightuserdata(pLS, Cloak);
		return 1;
	}
	else
		return 0;
...
inline int lua_GetCloakBonus(lua_State *pLS)
{
	bool bSuccess = true;
	int x = 5;
	double y = 1.20;
...
End:
	if (bSuccess)
	{
		...
		lua_pushnumber(pLS, stat);

		return 1;
	}
	else
		lua_pushnumber(pLS, 0); return 0;
}

inline int lua_UpgradeCloakLevel(lua_State *pLS)
{T_B
	short id = 9205;
	short level = 0;
	bool bSuccess = true;
	...
	pSItem = (SItemGrid *)lua_touserdata(pLS, 2); // target


	if (!pSItem)
	{
		pCha->BickerNotice("Cannot find admiral cloak!");
	}
	...
	{
		bSuccess = false;
		pCha->BickerNotice("Must be targeting a cloak!");
		g_CParser.DoString("UseItemFailed", enumSCRIPT_RETURN_NONE, 0, enumSCRIPT_PARAM_LIGHTUSERDATA, 1, pCha, DOSTRING_PARAM_END);
		goto End;
	}
	level = pSItem->GetInstAttr(ITEMATTR_MAXENERGY);
	level += 1;
End:
	if (bSuccess)
	{
		pSItem->SetInstAttr(ITEMATTR_MAXENERGY, level);
		int stat = (x * level / y);
		if (level > 0 && level <= 7)
		{
			for (short i = ITEMATTR_VAL_STR; i < ITEMATTR_VAL_LUK; i++)
			{
				pSItem->SetInstAttr(i, stat);
			}
		}
		...
		pCha->BickerNotice("Upgrade admiral cloak successful!");
		lua_pushnumber(pLS, 1);
	}
	else
		lua_pushnumber(pLS, 0);
...
inline int lua_SetItemForgeParam(lua_State *pLS)
{
	...
	{
		//unsigned long ulForgeP = (unsigned long)lua_tonumber(pLS, 3);;
		//short sHole = static_cast<short>(ulForgeP / 1000000000);
		// VC2003 VC2005巨大差别
		pSItem->SetDBParam(enumITEMDBP_FORGE, (unsigned long)lua_tonumber(pLS, 3));
	}
...
inline int lua_SaveProfile(lua_State *pLS)
{T_B
	// 输出信息
	MONITOR_DUMP
...
inline int lua_EnableProfile(lua_State *pLS)
{T_B
	int enable = lua_toboolean(pLS, 1);
...
///SetNameColor(role, color) - By Xeon
/*inline int lua_SetNameColor(lua_State *pLS)
{
	T_B
		bool	bSuccess = true; // Succesful function Identifier

	int nParaNum = lua_gettop(pLS); // Get Lua parameter count (role, color) = 2

	if (nParaNum != 2) // double check and make sure paramters are 2
	{
		cout << "SetNameColor: Param error!" << endl;
		bSuccess = false;
		goto End;
	}

	CCharacter *pCCha = (CCharacter*)lua_touserdata(pLS, 1); // (Gets characters role from lua)
	long lColorID = (long)lua_tonumber(pLS, 2); // Gets color value from lua
	long cColorID; // long value to hold color ID

	// switch and compare color IDs
	switch (lColorID)
	{
	case 1:
		cColorID = 1;
	...
	// if can't retrive character function fails, prints to GameServer command window
	if (!pCCha)
	{
		cout << "SetNameColor: Character error!" << endl; // Print to game server
		bSuccess = false; // Set function identifer flag to false (failed)
		goto End; // goto End of function
	}
// End of function label
End:
	if (bSuccess) // If lua function is successful
	{
		pCCha->setAttr(ATTR_EXTEND9, cColorID); // Set ATTR_COLOR
		pCCha->SynAttrToSelf(enumATTRSYN_REASSIGN); // SYNC attr to client
		lua_pushnumber(pLS, 1); // Push lua result (1 = SUCCESS)
	}
	else
		lua_pushnumber(pLS, 0); // Push lua result (0 = FAILED)

	return 1;
	T_E
}*/

inline int lua_LockItem(lua_State *pLS)
{T_B
...
End:
	if (bSuccess) // If lua function is successful
	{
		pCCha->SetNameColor(cColorID);
		pCCha->SyncCharNameColor(0);
		lua_pushnumber(pLS, 1);
	}
	else
		lua_pushnumber(pLS, 0); // Push lua result (0 = FAILED)
...
inline int lua_SetGuildColor(lua_State *pLS)
{
	T_B
		bool	bSuccess = true; // Succesful function Identifier
	...
	if (nParaNum != 2) // double check and make sure paramters are 2
	{
		cout << "SetGuildColor: Param error!" << endl;
		bSuccess = false;
		goto End;
	}
	...
	if (!pCCha)
	{
		cout << "SetGuildColor: Character error!" << endl; // Print to game server
		bSuccess = false; // Set function identifer flag to false (failed)
		goto End; // goto End of function
	}
	if (pCCha->GetGuildLeaderID() == 0)
	{
		bSuccess = false;
	}
	// End of function label
End:
	if (bSuccess) // If lua function is successful
	{
		game_db.GuildColor(pCCha, lColorID);
		pCCha->SetGuildColor(lColorID);
		...
		while (pGateServer = GETNEXTGATE())
		{
			...
			while (pCPlayer = (CPlayer *)GETNEXTPLAYER(pGateServer))
			{
				...
				if (pCPlayer->GetMainCha()->GetGuildID() == pCCha->GetGuildID())
				{
					pCPlayer->GetMainCha()->SetGuildColor(lColorID);
					pCPlayer->GetMainCha()->SyncCharNameColor(1);
				}
			}
...
inline int lua_SetGuildNameColor(lua_State *pLS)
{
	T_B
		bool	bSuccess = true; // Succesful function Identifier
	...
	if (nParaNum != 2) // double check and make sure paramters are 2
	{
		cout << "SetGuildNameColor: Param error!" << endl;
		bSuccess = false;
		goto End;
	}
...
End:
	if (bSuccess) // If lua function is successful
	{
		//pCCha->SynAttr(enumATTRSYN_REASSIGN);
		pCCha->setAttr(ATTR_EXTEND11, cColorID); // Set ATTR_COLOR
		pCCha->SynAttrToSelf(enumATTRSYN_REASSIGN); // SYNC attr to client
		lua_pushnumber(pLS, 1); // Push lua result (1 = SUCCESS)
	}
	else
...
//参数：角色，地图ID，副本编号
//返回值：2（1成功，副本编号）、1 （失败)
//Add by sunny.sun 20090812
#ifdef NEW_MAPCOPY
inline int lua_GetMapCopyIDByMapID(lua_State * pLS)
{
	bool	bSuccess = true;
	...
	int m_nMapCopyID = 0;
	if( !pCCha->Cmd_TeamFightMapCopy( (int)lua_tonumber(pLS, 2), (short)lua_tonumber(pLS, 3), m_nMapCopyID))
	{
		bSuccess = false;
		goto End;	
	}
...
End:
	...
	{
		lua_pushnumber(pLS, 1);
		lua_pushnumber(pLS, m_nMapCopyID);
		return 2;
	}
	else
	{
...
pCCha->GetKitbag()->SetChangeFlag((int)lua_tonumber(pLS, 2) != 0 ? true : false);
...
// 返回值：道具等级
/*
inline int lua_GetItemTradble(lua_State *pLS) {T_B
		bool success = false;
	int paramCount = lua_gettop(pLS);
	if (paramCount < 1) {
		printf("\n PARAM ERROR - GetItemTradble");
		success = false;
		goto End;
	}
	...
	{
		success = (pRec->chIsTrade > 0 ? true : false);
		goto End;
	}
End:
	if (success)
...
inline int lua_GetItemLv(lua_State *pLS)
{
	...
	if( CItemRecord::IsVaildFusionID( pCItemRec ) )
	{
		USHORT uItemID = pSItem->GetFusionItemID();
		CItemRecord	*pOldItem= GetItemRecordInfo( uItemID );
		if( pOldItem )
		{
			sItemLv = pOldItem->sNeedLv;
		}
	}
	else
...
inline int lua_GetKbCap(lua_State *pLS)
{
	...
	sCap = pCCha->GetKitbag()->GetCapacity();
...
inline int lua_Lua(lua_State *pLS)
{
	...
	lua_dostring(pLS, lua_tostring(pLS, 1));
...
{
	lua_dofile(L, GetResPath("script\\calculate\\skilleffect.lua"));
	return 1;
}
...
if (nParaNum != 2)
{
	...
	int issue = (int)lua_tonumber(pLS, 1);
	int max = (int)lua_tonumber(pLS, 2);

	string itemno;
	if( game_db.CalWinTicket(issue, max, itemno))
	{
		lua_pushstring(pLS, itemno.c_str());
		return 1;
	}
...
int state = (int)lua_tonumber(pLS, 2);
	...
	if(game_db.DisuseIssue(issue, state))
		return 1;
	return 0;
T_E}
...
//根据cha_id 获取 personinfo表中的生日
inline int lua_GetPersonBirthday( lua_State * pLS)
{T_B
	
	CCharacter *  person = (CCharacter*)lua_touserdata(pLS, 1);
	
	if( !person )
		return 0;
	int chaid = person->GetID();
	int birthday = 0;
	int month = -1;
	int day = -1;
	if( game_db.GetPersonBirthday( chaid, birthday ))
	{
		month =birthday >> 5;
		day = birthday - (birthday >> 5 << 5);
	}
	lua_pushnumber( pLS, month );
	lua_pushnumber( pLS, day );
		return 2;
T_E}

//Add by sunny.sun 20090313 离线时间
inline int lua_GetDisTime( lua_State * pLS )
{T_B
	int distime = 0;
	CCharacter *  person = (CCharacter*)lua_touserdata(pLS, 1);
	if( !person )
		return 0;
	if( !game_db.GetDisTime( person, distime ))
	{
		return 0;
	}
	lua_pushnumber( pLS, distime );
	return 1;
T_E}

inline int lua_opencrystal(lua_State *pLS)
{T_B
	int enable = lua_toboolean(pLS, 1);
	g_StoreSystem.SetCrystalTradeOpen(enable);
return 0;
T_E}

//Add by sunny.sun 20090821
inline int lua_ChangeExternValue( lua_State * pLS )
{T_B
	int nParaNum = lua_gettop(pLS); // 取得参数个数
	if (nParaNum != 3 || ! lua_isuserdata( pLS, 1 ) || !lua_isstring( pLS, 2 ) || !lua_isnumber( pLS, 3 ) )
	{
		return 0;
	}
	CCharacter *  pCCha = (CCharacter*)lua_touserdata(pLS, 1);
	if( pCCha )
	{
		const char* szScriptName = (const char*)lua_tostring(pLS, 2);
		long	m_lValue = (long)lua_tonumber(pLS,3);
		if( strcmp( szScriptName,"") == 0 || !pCCha )
		{
			return 0;
		}
		WPacket 	l_wpk	=GETWPACKET();
		WRITE_CMD( l_wpk, CMD_MM_EXCHEXTERNVALUE );
		WRITE_LONG( l_wpk, pCCha->GetID() );
		WRITE_STRING( l_wpk, szScriptName );
		WRITE_LONG( l_wpk, m_lValue );
		pCCha->ReflectINFof( pCCha,l_wpk );
		return 1;
	}
	return 0;
T_E}
...
REGFN(GetDisTime);
REGFN(ChangeExternValue)
REGFN(UpgradeCloakLevel);
REGFN(CleanMapFlag);
REGFN(UpdateWinnum);
...
REGFN(GetPersonBirthday);

REGFN(ScrollNotice);
REGFN(GMNotice);
REGFN(GetsPremissSkill);
// End
T_E}
