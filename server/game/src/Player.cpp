...
bool CPlayer::SynGuildBank(CKitbag* bag, char chType) {
	...
	//int canSeeBank = (m_pCtrlCha->guildPermission >= emGldPermViewBank || m_pCtrlCha->guildPermission == -1) ? 1 : 0;
	//m_pCtrlCha->SystemNotice("%d,%d, %d and %d = %d", m_pCtrlCha->guildPermission, emGldPermViewBank, m_pCtrlCha->guildPermission, emGldPermViewBank, (m_pCtrlCha->guildPermission & emGldPermViewBank));
	WPACKET WtPk = GETWPACKET();
	...
	// GUILD_BANK: Switched the first argument from *bags to bag
	/*if (canSeeBank != 1) {
		CKitbag* emptyBag;
