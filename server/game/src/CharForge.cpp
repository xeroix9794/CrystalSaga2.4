...
namespace mission
{
	...
	void CForgeSystem::ForgeItem( CCharacter& character, BYTE byIndex )
	{
		...
		SItemGrid *pItemData;
		if( !(pItemData = character.GetKitbag()->GetGridContByID( byIndex )) )
		{
		...
		for( int i = 0; i < FORGE_MAXNUM_ITEM; i++ )
		{
			...
			if( !character.HasItem( pRecord->ForgeItem[i].sItem, pRecord->ForgeItem[i].byNum ) )
			{
				...
				if( pForgeItem )
				{
					...
					strncpy_s( szForgeItem, sizeof(szForgeItem), pForgeItem->szName, _TRUNCATE );
				}
		...
		if( bSuccess )
		{
			...
			SItemGrid* pGrid = character.GetKitbag()->GetGridContByID( byIndex );
