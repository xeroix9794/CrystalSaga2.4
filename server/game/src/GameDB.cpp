 ...
 bool CTableAccountDetails::AddRum(CPlayer *pPlayer, DWORD rum)
{
	 ...
	 char buf[512] = { 0 };
	 _snprintf_s(buf, sizeof(buf), _TRUNCATE, "DRIVER={SQL Server};SERVER=%s;UID=%s;PWD=%s;DATABASE=AccountServer", g_Config.m_szDBIP, g_Config.m_szDBUsr, g_Config.m_szDBPass);
...
{T_B
...
#else
	char param[] = "act_id, guild_stat, guild_id, hp, sp, exp, radius, angle, cha_name, motto, icon, version, pk_ctrl, degree, job, gd, ap, tp, str, dex, agi, con, sta, luk, sail_lv, sail_exp, sail_left_exp, live_lv, live_exp, live_tp, main_map, map_x, map_y, birth, look, skillbag, shortcut, mission, misrecord, mistrigger, miscount, login_cha, kitbag, kitbag_tmp, map_mask, skill_state, bank, kb_locked, credit, store_item, extend, color, guild_permission";
#endif
	...
	if (DBOK(r) && r1 > 0)
	{
		...
		pCha->guildPermission = stoull(g_buf[nIndex++].c_str());
...
bool CTableGuild::InitGuildAttributes() {
	...
	{
		LG("lua_ai", "consult apply consortia process memeberODBC interface transfer error,position ID:%d\n", e);
	}
	catch (...)
	{
		LG("lua_ai", "Unknown Exception raised when initguildattributes\n");
	}
...
bool CTableGuild::ListTryPlayer(CCharacter* pCha, char disband_days)
{
	...
	//printf("\n Money : %lld", _atoi64(buf[7].c_str()));
	LLong	l_exp	=_atoi64(buf[8].c_str());
	//printf("\n EXP : %lld", _atoi64(buf[8].c_str()));
...
bool CTableGuild::Color(CCharacter* pCha, uLong colorId)
{
	...
	GetGuildLeaderID(pCha);
	if (pCha->GetGuildLeaderID() == cha_id)
	{
...
void PermAll(int cha_id, int permission, int guild_id)
{
	WPACKET pk = GETWPACKET();
	pk.WriteCmd(CMD_MM_UPDATEGUILDPERMISSONS);
	pk.WriteLong(guild_id);
	pk.WriteLong(cha_id);
	pk.WriteShort(permission);
	SENDTOWORLD(pk);
}
...
bool CTableCha::SetGuildPermission(int cha_id, unsigned long permission, int guild_id)
{
	...
	PermAll(cha_id, permission, guild_id);

	//printf("\n SET GUILD DB PERMISSION %d", permission);
...
int CTableCha::GetGuildPermission(int cha_id,  int guild_id)
{

	string buf[1];
	char filter[80];
	int permission = 0;
	char*param = "guild_permission";
	//sprintf(filter, "guild_id =%d",pChar->GetValidGuildID());
	_snprintf_s(filter, sizeof(filter), _TRUNCATE, "guild_id=%d and cha_id=%d", guild_id, cha_id);

	int	 l_retrow = 0;
	bool l_ret = _get_row(buf, 1, param, filter, &l_retrow);
	if (l_retrow == 1)
	{
		permission = atoi(buf[0].c_str());
		return permission;
	}
	else
		return 0;
