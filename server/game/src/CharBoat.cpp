...
{	
	...
	{T_B
		...
		if( pszName )
		{
			...
			strncpy_s( szBerth, sLen, pszName, _TRUNCATE );
		}
	T_E}
	...
	{T_B
		...
		strncpy_s( szData,sizeof(szData), RES_STRING(GM_CHARBOAT_CPP_00044),_TRUNCATE);
		BOOL bRet = TRUE;
		USHORT sPf = (USHORT)owner.getAttr( ATTR_JOB );
		for( BYTE i = 0; i < pInfo->sNumPfLimit; i++ )
		{
			//strcat( szData, g_GetJobName( pInfo->sPfLimit[i] ) );
			strncat_s( szData,sizeof(szData),  g_GetJobName( pInfo->sPfLimit[i] ), _TRUNCATE);
	...
	{T_B
		...
		if( owner.GetKitbag()->IsLock() || !owner.GetActControl(enumACTCONTROL_ITEM_OPT) )
		{
        ...
        if( owner.GetKitbag()->IsPwdLocked() )
        {
		...
		//lgtool_printBoat( RES_STRING(GM_CHARACTER_CPP_00139),	&owner	);
	...
	BOOL CCharBoat::GetBoatInfo( CCharacter& owner, DWORD dwBoatID )
	{
		...
		strncpy_s( AttrInfo.szName, sizeof(AttrInfo.szName), pBoat->GetName(), _TRUNCATE );
	...
	{T_B
		...
		if( pszData )
		{
			...
			strncpy_s( Data.szName, sizeof(Data.szName), pszData, _TRUNCATE );
		...
		if( !owner.TakeMoney( "", Info.dwMoney ) )
		{
			...
			param.setString( 0, pInfo->szName);
			param.setLong( 1, Info.dwMoney );
			RES_FORMAT_STRING( GM_CHARBOAT_CPP_00024, param, szData );
			owner.SystemNotice( szData );
			return TRUE;
		}
		...
		pBoat->GetKitbag()->SetCapacity( Data.sCapacity );
		...
		// 存入实例化物品
		owner.GetKitbag()->SetChangeFlag(false);
		...
		if( !game_db.SaveBoat( *pBoat, enumSAVE_TYPE_TIMER ) )
		{
			...
			char szData[128];
			CFormatParameter param(2);
			param.setString( 0, owner.GetGuildName());
			param.setString( 1, pBoat->GetGuildName());
			RES_FORMAT_STRING( GM_CHARBOAT_CPP_00031, param, szData );
			owner.SystemNotice( szData );
		...
		//	2008-8-21	yangyinyu	add	begin!
		//lgtool_printBoat(	RES_STRING(GM_CHARACTER_CPP_00140),	&owner	);
		//	2008-8-21	yangyinyu	add	end!
		...
		_snprintf_s(szLogName,sizeof(szLogName),_TRUNCATE, "Cha-%s+%u", pBoat->GetName(), pBoat->GetID());
	...
	{T_B
		...
		if( boat.getAttr( ATTR_HP ) < 0 )
		{
			...
			LG( "boatattr_error", "SetBoatAttr: boat currently HP value error,automatism resume!HP = %d\n", (long)boat.getAttr( ATTR_HP ) );
		...
		if( boat.getAttr( ATTR_SP ) < 0 )
		{
			...
			LG( "boatattr_error", "SetBoatAttr: boat currently SP value error,automatism resume!SP = %d\n", (long)boat.getAttr( ATTR_SP ) );
		...
		boat.GetKitbag()->SetCapacity( Info.sCapacity );
	...
	{T_B
		USHORT sNum = owner.GetKitbag()->GetUseGridNum();
		SItemGrid	*pGridCont;
		for( int i = 0; i < sNum; i++ )
		{
			pGridCont = owner.GetKitbag()->GetGridContByNum( i );
			if( pGridCont )
			{
				...
				if( !pItem )
				{
					...
					CFormatParameter param(2);
					param.setString( 0, owner.GetGuildName());
					param.setDouble( 1, pGridCont->sID );
					RES_FORMAT_STRING( GM_CHARBOAT_CPP_00033, param, szData );
					owner.SystemNotice( szData );
					continue;
				}
		...
		//	2008-8-21	yangyinyu	add	begin!
		//lgtool_printBoat(	RES_STRING(GM_CHARACTER_CPP_00141),	&owner	);
		//	2008-8-21	yangyinyu	add	end!
	...
	{T_B
		...
		if( !game_db.GetBoat( *pBoat ) )
		{
			...
			char szData[128];
			CFormatParameter param(2);
			param.setString( 0, owner.GetName() );
			param.setLong( 1, dwBoatID );
			RES_FORMAT_STRING( GM_CHARBOAT_CPP_00035, param, szData );
			owner.SystemNotice( szData );
		...
		if (chType == 1)
			...
			pBoat->m_CSkillState.ResetSkillState();
		...
		if( !owner.GetPlayer()->AddBoat( *pBoat ) )
		{
			...
			param.setDouble( 1, dwBoatID );
			param.setString( 2, owner.GetName());
			RES_FORMAT_STRING( GM_CHARBOAT_CPP_00036, param, szData );
			owner.SystemNotice( szData );
		...
		char szData[128];
		CFormatParameter param(2);
		param.setDouble( 0, owner.GetPlayer()->GetNumBoat());
		param.setString( 1, pBoat->GetName());
		RES_FORMAT_STRING( GM_CHARBOAT_CPP_00037, param, szData );
		owner.SystemNotice( szData );
