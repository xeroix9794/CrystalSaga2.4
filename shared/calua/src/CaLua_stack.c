...
void* PopRegTable(int regStruct)
{
	...
	for(i = 0; i < structs[regStruct]->numMembers; i++)
	{
		...
		case CLU_ULINT:
			{
				dPointers[i] = CLU_Calloc(sizeof(unsigned long int));
				*(unsigned long int*)dPointers[i] = (unsigned long int)lua_tonumber(virtualMachine, -1);
			}break;
		case CLU_FLOAT :
			{
...
void* PopValuesBySig(char* desc)
{
	...
	while(nToken > 0)
	{
		...
		{
			dPointers[nMembers - 1] = CLU_Calloc(sizeof(int));
			*(int*)dPointers[nMembers - 1] = (unsigned long int)lua_tonumber(virtualMachine, -1);
		}break;
		case CLU_FLOAT :
			{
...
int PushStructure(int regStruct, void* data)
{
	...
	for(i = 0; i < st->numMembers; i++)
	{
		...
		case CLU_ULINT:
		{
			lua_pushnumber(virtualMachine, (double)(*(unsigned long int*)marker));
		}break;
		case CLU_FLOAT:
			{
...
void* ConvertLuaStackData(int pos, int cType, int* size)
{
	...
	case CLU_ULINT:
	{
		...
		*size = sizeof(int);
		...
		case LUA_TNUMBER:						//Makes perfect sense
		{
			*(int*)ret = (unsigned long int)lua_tonumber(virtualMachine, pos);
		}
		break;
		case LUA_TBOOLEAN:						//Just a 1 or a zero
		{
			*(int*)ret = (unsigned long int)lua_toboolean(virtualMachine, pos);
		}
		break;
		case LUA_TNIL:							//Just pass it as zero
		{
		...
		default:								//Nothing else makes sense
		{
			CLU_Error("Unable to convert from Lua type %s to C type 'unsigned long int'", lua_typename(virtualMachine, lua_type(virtualMachine, pos)));
			SAFE_FREE(ret);
			return(NULL);
		}
		break;
...
int	CToLuaData(int type, void* data)
{
	...
	case CLU_ULINT:		{lua_pushnumber(virtualMachine, (double)(*(unsigned long int*)data)); }break;
