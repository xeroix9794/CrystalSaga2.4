...
bool cfl_db::handle_err(SQLHANDLE h, SQLSMALLINT t, RETCODE r, char const* sql /* = NULL */, bool reconn /* = false */) {
...
#define DBLOG printf
	...
	if (sqlret == SQL_ERROR || sqlret == SQL_INVALID_HANDLE) {
		return false;
	}
...
bool cfl_db::_connect(string& errinfo) {
	...
	do {
		...
		sqlret = SQLSetEnvAttr(_henv, SQL_ATTR_ODBC_VERSION, (SQLPOINTER)SQL_OV_ODBC3, SQL_IS_INTEGER);
		if (DBFAIL(sqlret)) {
		...
		sqlret = SQLDriverConnect(_hdbc, NULL, (SQLCHAR *)_connstr.c_str(), SQL_NTS, (SQLCHAR *)outbuf, sizeof outbuf, &outlen, SQL_DRIVER_NOPROMPT);
		if (DBFAIL(sqlret)) {
		...
		sqlret = SQLSetConnectAttr(_hdbc, SQL_ATTR_CONNECTION_TIMEOUT, (void *)10, 0);
...
bool cfl_db::connect(char* source, char* userid, char* passwd, string& err_info) {
	...
	do {
		...
		sqlret = SQLConnect(_hdbc, (SQLCHAR *)source, SQL_NTS, (SQLCHAR *)userid, SQL_NTS, (SQLCHAR *)passwd, SQL_NTS);
		if (DBFAIL(sqlret)) {
			handle_err(_hdbc, SQL_HANDLE_DBC, sqlret);
...
bool cfl_db::connect(char* servername, char* database, char* userid, char* passwd, string& err_info) {
	if (_connected) {
		err_info = "Already connected\n";
		return false;
	}
	...
	_snprintf_s(conn_str, sizeof(conn_str), _TRUNCATE, "DRIVER={SQL Server};SERVER=%s;UID=%s;PWD=%s;DATABASE=%s", servername, userid, passwd, database);
...
bool cfl_db::begin_tran() {
	...
	SQLRETURN sqlret = SQLSetConnectAttr(_hdbc, SQL_ATTR_AUTOCOMMIT, (SQLPOINTER)SQL_AUTOCOMMIT_OFF, SQL_IS_POINTER);
...
bool cfl_db::commit_tran() {
	...
	SQLRETURN sqlret = SQLSetConnectAttr(_hdbc, SQL_ATTR_AUTOCOMMIT, (SQLPOINTER)SQL_AUTOCOMMIT_ON, SQL_IS_POINTER);
	return (DBFAIL(sqlret)) ? false : true;
#endif
...
cfl_rs::cfl_rs(cfl_db* db, char const* tbl_name, int max_col) : _db(db), _hdbc(SQL_NULL_HDBC), _hstmt(SQL_NULL_HSTMT), _tbl_name(tbl_name) {
	attach_db(db);
	db->add(this);
...
int cfl_rs::get_affected_rows() {
	...
	if (!l_ret) {
		...
		return -2;	// Get the number of rows wrong
	}
...
RECONNECT:
	...
	do {
		try {
			...
			if (g_cchLogUtilDb == 1) {
				LG2("util_db", "exception raised from _get_row alloc\n");
			}
			break; 
		}
		...
		try {
			...
			if (g_cchLogUtilDb == 1) {
				LG2("util_db", "exception raised from _get_row bind\n");
			}
			break;
		}
		...
		try {
			...
			if (g_cchLogUtilDb == 1) {
				LG2("util_db", "exception raised from _get_row fetch\n");
			}
			break;
		}
		...
		try {
			...
			if (g_cchLogUtilDb == 1) {
				LG2("util_db", "exception raised from _get_row copydata\n");
			}
			break;
		}
	...
	try {
		...
		if (g_cchLogUtilDb == 1) {
			LG2("util_db", "exception raised from _get_row freestmt\n");
		}
		ret = false;
	}
...
bool cfl_rs::_get_row(string field_text[], int field_max_cnt, char* param, char* filter, int* affect_rows /* = NULL */) {
	bool ret = false;
	char sql[SQL_MAXLEN];
	int i = 0;
	try {
...
bool cfl_rs::_get_row3(string field_text[], int field_max_cnt, char* param, char* filter, int* affect_rows /* = NULL */) {
	bool ret = false;
	char sql[SQL_MAXLEN];
	int i = 0;
	try {
		...
		if (g_cchLogUtilDb == 1) {
			LG2("util_db", "exception raised from _get_row3 exec\n");
		}
		return false;
	}
...
RECONNECT:
	...
	do {
		try {
			...
			if (g_cchLogUtilDb == 1) {
				LG2("util_db", "exception raised from _get_row3 alloc\n");
			}
			break;
		}
		...
		try {
			...
			if (g_cchLogUtilDb == 1) {
				LG2("util_db", "exception raised from _get_row3 bind\n");
			}
			break;
		}
		...
		try {
			...
			if (g_cchLogUtilDb == 1) {
				LG2("util_db", "exception raised from _get_row3 fetch\n");
			}
			break;
		}
		...
		try {
			...
			if (g_cchLogUtilDb == 1) {
				LG2("util_db", "exception raised from _get_row3 copydata\n");
			}
			break;
		}
		ret = true;
	...
	try {
		...
		if (g_cchLogUtilDb == 1) {
			LG2("util_db", "exception raised from _get_row3 freestmt\n");
		}
		ret = false;
	}
...
bool cfl_rs::_get_rowOderby(string field_text[], int field_max_cnt, char* param, char* filter, int* affect_rows/* = NULL*/) {
	bool ret = false;
	char sql[SQL_MAXLEN];
	int i = 0;
	try {
...
RECONNECT:
	...
	do {
		...
		sqlret = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, len - 1, 0, (SQLPOINTER)pdata, len, &data_len);
		if (DBFAIL(sqlret)) {
			handle_err(hstmt, SQL_HANDLE_STMT, sqlret);
			break;
		}
...
RECONNECT:
	...
	do {
		...
		sqlret = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_BINARY, SQL_BINARY, len, 0, (SQLPOINTER)pdata, 0, &data_len);
		if (DBFAIL(sqlret)) {
			handle_err(hstmt, SQL_HANDLE_STMT, sqlret);
			break;
		}
...
bool cfl_rs::get_page_data(char* tablename, char* param, int pagesize, int pageindex, char* filter, char* sort, int sorttype, int& totalpage, int& totalrecord, vector< vector< string > > &data, unsigned short timeout) {
	bool ret;
...
RECONNECT:
	...
	do {
		try {
			...
			if (g_cchLogUtilDb == 1) {
				LG2("util_db", "exception raised from get() alloc\n");
			}
			break;
		}
		...
		try {
			...
			if (g_cchLogUtilDb == 1) {
				LG2("util_db", "exception raised from get() exec\n");
			}
			break;
		}
...
bool friend_tbl::get_friend_dat(friend_dat* farray, int& array_num, unsigned int cha_id, bool* drop /* = NULL */) {
	...
	try {
		do {
			...
			array_num = i; // The number of rows fetched
...
// Returns if successful conversion0
int Util::ConvertDBParam(const char* param, char* buf, size_t size, size_t& count) {
	...
	// Leave the last character save \ 0
	if (len >= size) {
		return -1;
	}
