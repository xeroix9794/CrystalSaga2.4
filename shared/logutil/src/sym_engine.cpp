...
unsigned sym_engine::symbol(char * buf, unsigned len, unsigned * pdisplacement) {
	if (!len || !buf || IsBadWritePtr(buf, len) || (pdisplacement && IsBadWritePtr(pdisplacement, sizeof(unsigned)))) {
		return 0;
	}
...
unsigned sym_engine::fileline(char * buf, unsigned len, unsigned * pline, unsigned * pdisplacement) {
	if (!len || !buf || IsBadWritePtr(buf, len) || (pline && IsBadWritePtr(pline, sizeof(unsigned))) || (pdisplacement && IsBadWritePtr(pdisplacement, sizeof(unsigned)))) {
		return 0;
	}
...
bool sym_engine::stack_first(CONTEXT* pctx) {
	if (!pctx || IsBadReadPtr(pctx, sizeof(CONTEXT))) {
		if (pctx == NULL{
			g_os << "pctx = NULL, stack_first return false!\n";
		}
		else {
			g_os << "bad Read Ptr, stack_first return false!\n";
		}
		return false;
	}
...
sym_engine::guard::guard() : m_ref(0) {
}
		...
		if (::SymInitialize(hProc, 0, TRUE)) {
			...
			if (IsNT()) {
				...
				if (hInst) {
					...
					DWORD cbNeeded = 0;
					if (fnEnumProcessModules && fnEnumProcessModules(GetCurrentProcess(), 0, 0, &cbNeeded) && cbNeeded) s{
...
static DWORD WINAPI tproc(void * pv) {
	...
	__try {
		...
		IP("sym_engine::tproc, spin count %u\n", debug_only);
#endif;
...
#ifdef SYM_ENGINE_TRACE_SPIN_COUNT
			IP("sym_engine::stack_trace, spin count %u\n", debug_only);
#endif
...
bool sym_engine::stack_trace(sym_engine& sym, CONTEXT * pctx, unsigned skip, const char * fmt) {
	if (!g_os.is_open()) {
		std::string	strfile;
		LG_GetDir(strfile);
