...
void TDt::Format(char* ach, int size, int a_nDisplayFormat) {
	...
	strncpy_s(ach, size, s.c_str(), _TRUNCATE);
...
TDt& TDt::Set(const char* a_p) {
	...
	char p[16];
	strncpy_s(p, sizeof(p), a_p, _TRUNCATE);
...
void CSQLBaseWhere::SetWhere(const char* columnName, const char* a_value, char a_DataType, bool a_bUseLike, bool a_bUseOr) {
	if (_where != "") {
...
CSQLDatabase::CSQLDatabase() : _isOpen(false), _rc(SQL_SUCCESS), _henv(0), _hdbc(0), _hstmt(0), _autoCommit(true) {
...
string& CSQLDatabase::dbmsName() {
	...
	_rc = SQLGetInfo(_hdbc, SQL_DBMS_NAME, InfoValuePtr, BufferLength, &StringLengthPtr);
...
bool CSQLDatabase::SQLConnect(const char * a_szDSN, const char* a_szUserName, const char* a_szPassword) {
	...
	_rc = ::SQLConnect(_hdbc, (SQLCHAR*)a_szDSN, SQL_NTS, (SQLCHAR*)a_szUserName, SQL_NTS, (SQLCHAR*)a_szPassword, SQL_NTS);
...
bool CSQLDatabase::SQLConnect(char const* conn_instr) {
	...
	// end former constructor code
	_rc = ::SQLDriverConnect(_hdbc, NULL, (SQLCHAR *)conn_instr, SQL_NTS, (SQLCHAR *)conn_outstr, SQL_NTS, &outstr_len, SQL_DRIVER_NOPROMPT);
...
void CSQLDatabase::ThrowError(SQLSMALLINT handleType) {
	...
	rc = SQLGetDiagRec(handleType, theHandle, 1 /* recnumber? */, szSQLState, &nNativeError, szErrorMsg, cbErrorMsgMax, &cbErrorMsg);
...
// supports ROLLBACK and COMMIT
bool CSQLDatabase::SupportsTransactions() {
	SQLUINTEGER result;
	SQLRETURN rc = ::SQLGetInfo(_hdbc, SQL_TXN_CAPABLE, (SQLPOINTER)&result, 0, 0);
	return result != SQL_TC_NONE;
}

// makes all transactions COMMIT immediately
void CSQLDatabase::SetAutoCommit(bool tf) {
	_autoCommit = tf;
	if (_autoCommit == true) {
		return;
	}
...
CSQLDelete::CSQLDelete() : CSQLBaseWhere() {
}

CSQLDelete::CSQLDelete(const char* table) : CSQLBaseWhere() {
	SetTable(table);
}
...
CSQLInsert::CSQLInsert() : CSQLBase() {
}

CSQLInsert::CSQLInsert(const char* tableName) : CSQLBase() {
	SetTable(tableName);
}
...
void CSQLInsert::SetColumn(const char* columnName, const char* a_chValue, char a_DataType) {
	...
	else if (_stricmp(a_chValue, "NULL") == 0) {
		_values += "NULL";
	}
	else if (strlen(a_chValue) >= 3 && a_chValue[0] == '{' && a_chValue[1] == 'd' && a_chValue[2] == '\'') {
		_values += a_chValue;
	}
	else if (_stricmp(a_chValue, "now()") == 0) {
		_values += a_chValue;
	}
...
void CSQLInsert::SetColumn(const char* columnName, double value) {
	...
	char ach[32];
	_snprintf_s(ach, sizeof(ach), _TRUNCATE, "%.2f", value);
...
void CSQLInsert::SetColumn(const char* columnName, int value) {
	...
	char ach[32];
	_snprintf_s(ach, sizeof(ach), _TRUNCATE, "%d", value);
...
CSQLRecordset::CSQLRecordset() : CSQLBaseWhere(), _rc(SQL_SUCCESS), _hdbc(NULL), _hstmt(NULL) {
}

CSQLRecordset::CSQLRecordset(SQLHDBC a_hdbc) : CSQLBaseWhere(), _rc(SQL_SUCCESS), _hdbc(a_hdbc), _hstmt(NULL) {
	assert(a_hdbc != NULL);
}
...
CSQLRecordset::CSQLRecordset(CSQLDatabase& db) : CSQLBaseWhere(), _rc(SQL_SUCCESS), _hdbc(db._hdbc), _hstmt(NULL) {
}
...
std::string CSQLRecordset::SQLGetData(int a_uRow, int a_eDataType) {
	...
	if (a_eDataType == DOUBLE) {
		char ach1[32];
		_snprintf_s(ach1, sizeof(ach1), _TRUNCATE, "%.2f", atof(ach));
		_result = ach1;
		return _result;
	}
...
void CSQLRecordset::ThrowError(bool useDbHandle) {
	...
	if (useDbHandle) {
		rc = SQLGetDiagRec(SQL_HANDLE_DBC, _hdbc, 1, szSQLState, &nNativeError, szErrorMsg, cbErrorMsgMax, &cbErrorMsg);
	}
	else {
		rc = SQLGetDiagRec(SQL_HANDLE_STMT, _hstmt, 1, szSQLState, &nNativeError, szErrorMsg, cbErrorMsgMax, &cbErrorMsg);
	}
	szSQLState[sizeof(szSQLState) - 1] = 0; // insurance
...
int CSQLRecordset::GetColumnType(int a_nColumn) {
	...
	SWORD cbScale;
	SWORD fNullable;
	_rc = ::SQLDescribeCol(_hstmt, a_nColumn, achColName, SQL_MAX_COLUMN_NAME_LEN, &cbColName, &fSQLType, &cbPrecision, &cbScale, &fNullable);
	if (!(_rc == SQL_SUCCESS || _rc == SQL_SUCCESS_WITH_INFO)) {
		return 0;
	}
	return fSQLType;
...
CSQLUpdate::CSQLUpdate() : CSQLBaseWhere() {
}

CSQLUpdate::CSQLUpdate(const char* table) : CSQLBaseWhere() {
	SetTable(table);
}
...
void CSQLUpdate::SetColumn(const char* columnName, long a_lValue, char a_DataType) {
	...
	if (a_DataType == CSQLBase::ODBC_DATE) {
		...
		char ach[32];
		_snprintf_s(ach, sizeof(ach), _TRUNCATE, "%ld", a_lValue);
		_columns += ach;
	}
}
