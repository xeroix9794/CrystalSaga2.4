...
static char const basis_64[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
static char const basis_128[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789~!@#$%^&*()_+|\0";
...
int base64(char const* src, unsigned int src_len, char* dst, unsigned int dst_max_len, unsigned int* dst_len) {
...
#if 1
bool encrypt_B(char* src, unsigned int src_len, char* key, unsigned int key_len, bool en /* = true */) { // 'B' encrypt/decrypt algorithm
...
void KCHAPc::GetStep1Data(char* buf, int buf_len, int& out_len) {
	...
	C3DES des;
	ctmp1_len = des.Encrypt(ctmp1, src, src_len, m_strPwdDig.c_str(), m_strPwdDig.length());
...
void KCHAPc::GetStep1Data(char* buf, int buf_len, int& out_len) {	
	...
	// encrypt
	CDES::RunDes(CDES::ENCRYPT, CDES::ECB, (char *)szPad, buf, uiPadLen, m_strPwdDig.c_str(), m_strPwdDig.length());

	out_len = uiPadLen;
}

void KCHAPc::GetSessionClrKey(void* pdat, int dat_len, char* buf, int buf_len, int& out_len) {
	CDES::RunDes(CDES::DECRYPT, CDES::ECB, (char *)pdat, buf, dat_len, m_strPwdDig.c_str(), m_strPwdDig.length());
...
bool KCHAPs::DoAuth(char const* name, char const* chap_string, char const* pdat, int dat_len, char const* passwd_digest) {
	bool ret;
	...
	if (ret) {
		...
		// authentication is successful, the session key is generated immediately
		NewSessionKey();
	}
...
bool C3DES::algo_A(char* out, char* in, long data_len, char const* key, int key_len, bool encrypt) {
	if (!(out && in && key && (data_len = (data_len + 7) & 0xfffffff8))) {
		return false;
	}
...
long C3DES::Encrypt(char* out, char const* in, long data_len, char const* key, int key_len) {
	algo_A(out, (char *)in, data_len, key, key_len, true);
	return 8 * (data_len / 8 + ((data_len % 8 != 0) ? 1 : 0));
}

long C3DES::Decrypt(char* out, char const* in, long data_len, char const* key, int key_len) {
...
void CDES::S_func(bool Out[32], const bool In[48]) {
	for (char i = 0, j, k; i < 8; ++i, In += 6, Out += 4) {
		...
		// Organize SID subscripts
		k = (In[1] << 3) + (In[2] << 2) + (In[3] << 1) + In[4];

		// The corresponding 4bit assignment
		for (int l = 0; l < 4; ++l) {
...
bool CDES::RunDes(bool bType, bool bMode, char* In, char* Out, unsigned datalen, const char* Key, const unsigned char keylen) {
	...
	// Determine the input legitimacy
	if (!(In && Out && Key && datalen && keylen >= 8)) {
		return false;
	}
	
	// Only deal with an integer multiple of 8, less than the length of their own fill
	...
	// Construct and generate SubKeys
	...
	// CBC mode
		...
		char cvec[8] = "";	// Twist vector
		char cvin[8] = "";	// Intermediate variables
		...
		if (nKey == 1) {
			for (int i = 0, j = datalen >> 3; i < j; ++i, Out += 8, In += 8) {
				if (bType == CDES::ENCRYPT) {
					...
					// Divide the input with the torsion variable
					for (int j = 0; j < 8; ++j) {
						cvin[j] = In[j] ^ cvec[j];
					}
				}
				...
				if (bType == CDES::ENCRYPT) {
					...
					// Set the output as a torsion variable
					memcpy(cvec, Out, 8);
				}
				else {

					// Divide the output with the torsion variable
					for (int j = 0; j < 8; ++j) {
						Out[j] = Out[j] ^ cvec[j];
					}

					// Set the input as a torsion variable
					memcpy(cvec, cvin, 8);
				}
			}
		}
...
// Calculate and populate the subkey into the SubKey data
...
void CDES::DES(char Out[8], char In[8], const PSubKey pSubKey, bool Type) {
	...
	if (Type == ENCRYPT) {
		for (int i = 0; i < 16; ++i) {
			memcpy(tmp, Ri, 32);		// Ri[i - 1]					// Ssave
			F_func(Ri, (*pSubKey)[i]);	// Ri[i - 1]					// After conversion and SBox output is P
		...
		for (int i = 15; i >= 0; --i) {
			memcpy(tmp, Ri, 32);		// Ri[i - 1]					// save
			F_func(Ri, (*pSubKey)[i]);	// Ri[i - 1]					// After conversion and SBox output is P
			Xor(Ri, Li, 32);			// Ri[i] = P XOR Li[i - 1]
			memcpy(Li, tmp, 32);		// Li[i] = Ri[i - 1]
		}
	}
	RotateL(M, 64, 32);					// Ri and Li transposition recombination
	Transform(M, M, IPR_Table, 64);		// The final result is transformed
	BitToByte(Out, M, 64);				// Organized into characters
}
