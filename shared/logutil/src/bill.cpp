...
int CPAI::Login(char const* name, char const* pwd) {
	...
	strncpy_s((char*)chMD5_PWD, sizeof(chMD5_PWD), pwd, _TRUNCATE);
...
namespace HTTP {
	...
	public:
		...
		*/
		...
		std::string::size_type getHeaderSize() const;
		...
		*/
		...
		std::string::size_type getDataSize() const;
		...
		*/
		...
		std::string::size_type getFooterSize() const;
		...
		*/
		std::string::size_type getRawSize() const;
	...
	protected:
		...
		*/
		std::string::size_type findSize(std::string::size_type n, const std::string& source) const;
...
namespace HTTP {
	...
	bool GetRequest::request(const std::string& page) {
		...
		_snprintf_s(buffer, sizeof(buffer), cmd.c_str(), _TRUNCATE);
	...
	std::string GetRequest::getHeaderAsString(const std::string& key) const {
		if (m_header.size() > std::string::size_type(0)) {
			...
			std::string::size_type end = m_rawdata.find("\r\n", pos);
	...
	std::string::size_type GetRequest::getHeaderSize() const {
		return m_header.size();
	}

	std::string::size_type GetRequest::getDataSize() const {
		return m_data.size();
	}

	std::string::size_type GetRequest::getFooterSize() const {
		return m_footer.size();
	}

	std::string::size_type GetRequest::getRawSize() const {
	...
	bool GetRequest::isTransferFinished() const {
		...
		std::string::size_type pos = m_rawdata.find("\r\n\r\n", 0);
		if (pos == std::string::npos) {
			return false;
		}
		pos += 4;
	...
	bool GetRequest::postProcessData() {
		...
		std::string::size_type size, pos, prev;
	...
	std::string::size_type GetRequest::findSize(std::string::size_type n, const std::string& source) const {
		std::string::size_type end, crlf, semicolon, size;
		crlf = source.find("\r\n", n);
	...
	bool GetRequest::splitData() {
		...
		if (!isChunkedTransfer()) {
			...
			std::string::size_type size = getContentLength();
			...
			std::string::size_type end = m_rawdata.rfind("\r\n0", std::string::npos);
			if (end == std::string::npos) {
...
int PSS_GetCredit(char const* section, char const* name, std::string& strErr) {
	...
	// TO KILL 
	_snprintf_s(buf, sizeof(buf), _TRUNCATE, "moliyo/checklogin2kop.aspx?memberpk=%s&productfk=%s", name, section);
	...
	if (gr.connect("61.152.115.184", 8080)) {
		if (gr.request(buf)) {
			while (gr.process()) {}
			rc = atoi(gr.getData().c_str()); // The Data body is the return value
		}
		else {
			rc = 1;
		}
