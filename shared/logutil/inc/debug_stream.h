...
protected:
	int_type overflow(int_type ch = traits_type::eof()) {
		if (!traits_type::eq_int_type(traits_type::eof(), ch)) {
			...
			// m_buf.append(1, traits_type::to_char_type(ch));
...
template<> inline void basic_debugbuf<char, std::char_traits<char> >::output_debug_string(const char *text) {
	::OutputDebugStringA(text);
}

template<> inline void basic_debugbuf<wchar_t, std::char_traits<wchar_t> >::output_debug_string(const wchar_t *text) {
