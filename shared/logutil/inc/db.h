...
public:
	...
	bool connect(char* servername, char* database, char* userid, char* passwd, string& err_info);
...
public:
	...
	// Non-blocking (nolock)
	bool _get_row(string field_text[], int field_max_cnt, char* param, char* filter, int* affect_rows = NULL);

	// Default lock, space after the data cut off
	bool _get_row3(string field_text[], int field_max_cnt, char* param, char* filter, int* affect_rows = NULL);
	bool _get_rowOderby(string field_text[], int field_max_cnt, char* param, char* filter, int* affect_rows = NULL);
...
inline bool cfl_rs::handle_err(SQLHANDLE h, SQLSMALLINT t, RETCODE r, char const* sql /* = NULL */, bool reconn /* = false */) {
