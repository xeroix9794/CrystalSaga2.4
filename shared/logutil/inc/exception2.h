...
namespace {
	...
	struct call_stack {	
		call_stack (unsigned skip = 3) : m_s((skip == unsigned(-1)) ? "" : call_stack::get(skip)) {
		}
...
class exception2 : public std::exception, public call_stack  {
  public:
	  exception2(const __exString& s, unsigned skip = 4 /* -1 for disable call stack */) : std::exception(s), call_stack(skip) {
	  }
};
