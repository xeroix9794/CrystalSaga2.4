...
private:
	...
	// string _s; 
	// a place to put results from stuff
};
...
public:
	...
	bool ExecuteSQL(const std::string& s) {
		return ExecuteSQL(s.c_str()); 
	}
...
public:
	...
	bool SQLExecDirect(const std::string& stmt) { 
		return SQLExecDirect(stmt.c_str()); 
	}
...
protected:
	void ThrowError(bool useDbHandle = false);
