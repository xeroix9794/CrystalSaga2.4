...
template <typename P>
class exception_trap : public CAuxStdThunk < exception_trap > {
public:
	typedef P Policy;

protected:
	LPTOP_LEVEL_EXCEPTION_FILTER Sm_fn_prev_filter;
	Policy m_policy;
...
template <typename P>
inline exception_trap<P>::exception_trap(bool b, const Policy& p) : m_fn_prev_filter(0), m_policy(p) {
...
inline long __stdcall exception_trap<P>::filter(EXCEPTION_POINTERS * pex) {
	...
	__try {
		...
		if (r == EXCEPTION_CONTINUE_SEARCH && (m_fn_prev_filter && m_fn_prev_filter != fake()) && !IsBadCodePtr((FARPROC)m_fn_prev_filter)) {
