	...
	{																			//池任务队列初始化
		...
		try
		{
			...
			if(!m_mtxQue||!m_semQueAdd||!m_semQueGet)
				...
				THROW_EXCP(excpSync,"Ceeate sync mutex failed!");
		}catch(...)
		{
			m_mtxQue.unlock();
			throw;
...
public:
	TaskWait(uLong size =0):PreAllocStru(size)
	{
		...
		if(!m_semWait.Create(0,1,0))THROW_EXCP(excpSync,"Ceeate sync mutex failed(task wait)!");
	};
private:
	Sema					m_semWait;
...
class ThrdQue
{
	...
	ThrdQue():m_thread(0)
	{
		...
		if(!m_mtxPool||!m_mtxUpdate)
		{
			...
			THROW_EXCP(excpSync,"Ceeate sync mutex failed(ThrdQue)!");
		}
	};
