...
Sender	&Sender::operator<<(WPacket &wpk)
{
	...
	while(bool(wpk) && m_datasock->m_delflag ==0)
	{
		if(m_tail && m_tail->HasSpace())
		{
            ...
            {
                LogLine l_line(g_DebugSendLog);
                l_line <<newln<<"Sender," <<m_datasock->GetLocalIP() <<"(" << m_datasock->GetLocalPort() << ") ==> "<<m_datasock->GetPeerIP() << "("<<m_datasock->GetPeerPort() <<  "),cmd," << Test.ReadCmd() <<endln;
            }
#endif
			// End
