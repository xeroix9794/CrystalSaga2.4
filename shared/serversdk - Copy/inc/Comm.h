...
//外部调用
	...
	/**
	* @brif 连接前处理
	* @param[in]  DataSocket  Socket数据
	*
	* @return bool true-允许连接,false-不允许连接
	*/
	...
	*/
	...
	/**
	* @brif 连接后通知处理
	* reason值:0-本地程序正常退出;-1-Socket错误;-3-网络被对方关闭;-5-包长度超过限制;-7-阻塞太严重了;-9-KeepAlive失败
	* @param[in]  DataSocket  Socket数据
	* @param[in]  reason  Socket数据
	*/
...
public:
	...
	/**
	* @brif 连接服务器
	* @param[in]  hostname  服务器地址 
	* @param[in]  port 服务器端口
	* @param[in]  sock_out 连接端口
	*/
