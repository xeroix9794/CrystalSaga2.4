...
LogStream	dbc::g_DebugSendLog("DebugOut");
LogStream	dbc::g_DebugReceiveLog("DebugIn");
// End
...
void LogStream::Open(bool bakmode)
{
	...
	if(m_DirName || strlen(m_DirName)>0)
	{
		if(*m_DirName =='/' ||*m_DirName =='\\')
		{
			...
			::strncpy_s(l_path, sizeof(l_path), m_DirName+1, _TRUNCATE);
		}else
		{
            //strcpy(l_path,m_DirName);
			::strncpy_s(l_path, sizeof(l_path), m_DirName, _TRUNCATE);
		}
		int		l_dircount	=0;
		cChar	*l_dir =l_path;
			...
			*l_dim	=0;
			if(strlen(l_dir) >0)
			{
				if(l_dircount ==1)
				{
					...
					::strncpy_s(l_frmstr, sizeof(l_frmstr), l_path, _TRUNCATE);
					//strcat(l_frmstr,"%04d-%02d-%02d %02d.%02d.%02d.%03d");
					::strncat_s(l_frmstr, sizeof(l_frmstr), "%04d-%02d-%02d %02d.%02d.%02d.%03d", _TRUNCATE);
					SYSTEMTIME l_st;
					::GetLocalTime(&l_st);
					//sprintf(l_olddir,l_frmstr,l_st.wYear,l_st.wMonth,l_st.wDay,l_st.wHour,l_st.wMinute,l_st.wSecond,l_st.wMilliseconds);
					::_snprintf_s(l_olddir,sizeof(l_olddir),_TRUNCATE, l_frmstr,l_st.wYear,l_st.wMonth,l_st.wDay,l_st.wHour,l_st.wMinute,l_st.wSecond,l_st.wMilliseconds);
		...
		//strcat(l_path,"/");
		strncat_s(l_path, sizeof(l_path), "/", _TRUNCATE);
	}
	//strcat(l_path,m_LogFileName);
	strncat_s(l_path, sizeof(l_path),m_LogFileName, _TRUNCATE);
	//strcat(l_path,".log");
	strncat_s(l_path, sizeof(l_path),".log", _TRUNCATE);
	if(bakmode)
	{
...
void LogStream::Backup()
{
	...
	strncpy_s(l_frmstr, sizeof(l_frmstr), "%04d-%02d-%02d %02d.%02d.%02d.bak", _TRUNCATE);
	SYSTEMTIME l_st;
	::GetLocalTime(&l_st);
	//sprintf(l_bakdir,l_frmstr,l_st.wYear,l_st.wMonth,l_st.wDay,l_st.wHour,l_st.wMinute,l_st.wSecond);
	::_snprintf_s(l_bakdir, sizeof(l_bakdir), _TRUNCATE, l_frmstr,l_st.wYear,l_st.wMonth,l_st.wDay,l_st.wHour,l_st.wMinute,l_st.wSecond);
