	...
	for(int i=0;i<m_argc;i++){
		...
		if(!i){
         ...
         GetFullPathName(m_argv[i]+1,1024,m_argv[i]+1,0);
			...
			_strlwr_s(l_dstr.GetBuffer(), l_dstr.Size());
			if(l_dstr !=".exe")
			{				//判断末尾是否有.exe,没有就追加上
				//strcat(m_argv[i],".exe");
				::strncat_s(m_argv[i], sizeof(l_cmdline) - l_pos, ".exe", _TRUNCATE );
			}
			//strcat(m_argv[i],"\"");
			::strncat_s(m_argv[i], sizeof(l_cmdline) - l_pos, "\"", _TRUNCATE );
		}else
		{
...
//分析参数
	...
	for(long i=1;i<m_argc;i++){
		...
		_strlwr_s(l_dstr.GetBuffer(), l_dstr.Size());			//转换成小写
