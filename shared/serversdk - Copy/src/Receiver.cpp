...
#if 0
       LogLine l_line(g_DebugReceiveLog);
       l_line <<newln<<"Sender," <<m_datasock->GetLocalIP() <<"(" << m_datasock->GetLocalPort() << ") <== "<<m_datasock->GetPeerIP() << "("<<m_datasock->GetPeerPort() <<  "),cmd," << m_rpk.ReadCmd() <<endln;
#endif
	   // End
...
{
    //°²È«¼ì²â Add by Waiting 2009-06-18
	if( !m_datasock )
		return 0;
		...
		if(l_reallen >0)
		{
			...
			while(!m_datasock->m_delflag)
			{
				if(!m_rpk.GetPktLen()&&(m_p >=_len_inc))
				{
					...
					{
					    LogLine l_line(g_DebugReceiveLog);
					    l_line <<newln<<"Sender," <<m_datasock->GetLocalIP() <<"(" << m_datasock->GetLocalPort() << ") <== "<<m_datasock->GetPeerIP() << "("<<m_datasock->GetPeerPort() 
							<<  "),cmd," << m_rpk.ReadCmd() << ",len," << m_rpk.GetPktLen() << ",MaxLen" << __tca->__pkt_maxlen <<endln;
