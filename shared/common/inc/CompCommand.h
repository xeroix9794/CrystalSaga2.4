...
enum EActionType // server,client 之间的行动类型
{
	...
	enumACTION_REQUESTGUILDLEVEL,
	enumACTION_REQUESTAGUILDATTR,

	enumACTION_TOTAL_ITEM_PICK,//多道具拾取
...
enum EItemOperateResult
{
	...
	enumITEMOPT_ERROR_ITEMLOCK, // 道具处于锁定状态 add by ning.yan 2008-11-11 
};
...
typedef
struct 
	stNetChangeChaPart		// 改变角色外观,注:客户端-服务器端默认皮肤的约定:客户端接收到0的皮肤时,自动载入CharacterRecord中的皮肤
{
...
#define SHORTCUT_NUMS false

// 快捷栏
const DWORD MAX_FAST_ROW = 3;
const DWORD MAX_FAST_COL = 12;
//#if SHORTCUT_NUMS
//const DWORD SHORT_CUT_NUM = MAX_FAST_ROW * MAX_FAST_COL + 10;
//#else
