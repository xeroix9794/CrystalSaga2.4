...
#pragma message("[SkillEff] Update CSkillStateRecordSet");
	pInfo->chID = (short)pInfo->nID;
	// 名称
	//strncpy(pInfo->szName, pInfo->szDataName, defSKILLSTATE_NAME_LEN);
	strncpy_s(pInfo->szName, sizeof(pInfo->szName), pInfo->szDataName, _TRUNCATE);
	...
	strncpy_s(pInfo->szOnTransfer, sizeof(pInfo->szOnTransfer), ParamList[m++].c_str(), _TRUNCATE);
	//pInfo->szOnTransfer[defSKILLSTATE_SCRIPT_NAME - 1] = '\0';
	// 使用效果脚本
	//strncpy(pInfo->szAddState, ParamList[m++].c_str(), defSKILLSTATE_SCRIPT_NAME);
	strncpy_s(pInfo->szAddState, sizeof(pInfo->szAddState), ParamList[m++].c_str(), _TRUNCATE);
	//pInfo->szAddState[defSKILLSTATE_SCRIPT_NAME - 1] = '\0';
	// 移除效果脚本
	//strncpy(pInfo->szSubState, ParamList[m++].c_str(), defSKILLSTATE_SCRIPT_NAME);
	strncpy_s(pInfo->szSubState, sizeof(pInfo->szSubState), ParamList[m++].c_str(), _TRUNCATE);
