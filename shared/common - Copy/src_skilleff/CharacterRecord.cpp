...
{T_B
	...
	_tcsncpy_s(pInfo->szIconName, sizeof(pInfo->szIconName), ConvertResString(ParamList[m++].c_str()), _TRUNCATE);
	// End
	...
	_tcsncpy_s(pInfo->szGuild, sizeof(pInfo->szGuild), ParamList[m++].c_str(), _TRUNCATE);
	//pInfo->szGuild[defCHA_GUILD_NAME_LEN - 1] = _TEXT('\0');
	// 角色称号
	//_tcsncpy(pInfo->szTitle, ParamList[m++].c_str(), defCHA_TITLE_NAME_LEN);
	_tcsncpy_s(pInfo->szTitle, sizeof(pInfo->szTitle), ParamList[m++].c_str(), _TRUNCATE);
	//pInfo->szGuild[defCHA_GUILD_NAME_LEN - 1] = _TEXT('\0');
	// 角色职业
	_tcsncpy_s(pInfo->szJob,sizeof(pInfo->szJob), ParamList[m++].c_str(), _TRUNCATE);
	//pInfo->szGuild[defCHA_GUILD_NAME_LEN - 1] = _TEXT('\0');
