...
public:
	...
	LONG64	GetAttr(long lNo);
	long	SetAttr(long lNo, LONG64 lVal);
	LONG64	GetAttrMaxVal(long lNo);
	long	DirectSetAttr(long lNo, LONG64 lVal); // 直接设置最大值，只用于客户端（因为客户端的数据来源于服务端，是合法数据）
	long	AddAttr(long lNo, LONG64 lVal);
...
inline LONG64 CChaAttr::GetAttrMaxVal(long lNo)
{T_B
	if (lNo >= ATTR_MAX_NUM)
		return -1;
...
inline long CChaAttr::SetAttr(long lNo, LONG64 lVal)
{T_B
	if (lNo >= ATTR_MAX_NUM) return 0;

	long	lRet = 2;
	if (lVal > __int64(g_lMaxChaAttr[lNo]))
	{
		lRet = 1;
...
inline long CChaAttr::DirectSetAttr(long lNo, LONG64 lVal)
{T_B
	if (lNo >= ATTR_MAX_NUM) return 0;
...
inline long CChaAttr::AddAttr(long lNo, LONG64 lVal)
{T_B
	if (lNo >= ATTR_MAX_NUM)
		return 0;
