...
int CCharacterModel::LoadPose()
{
	...
	if(_TypeID == 1)
	{
		drIAnimCtrlAgent* anim_agent = GetPhysique()->GetAnimCtrlAgent();
		if(anim_agent)
		{
			...
			drIAnimCtrlBone* ctrl_bone = (drIAnimCtrlBone*)ctrl_obj_bone->GetAnimCtrl();
			drIPoseCtrl* pc = ctrl_bone->GetPoseCtrl();
			drPoseInfo* pi = pc->GetPoseInfo(0);
...
void CCharacterModel::PlayPose( DWORD pose, DWORD type, int time, int fps, DWORD blend_flag, DWORD blend_src_num )
{
	...
	drPoseInfo* pi = c->GetPoseInfo(_SmallPoseID);
	if(pi == 0)
	{
...
void CCharacterModel::PlayPose( const drPlayPoseInfo* info )
{
	if( _ModelType != MODEL_CHARACTER )
	{
...
DWORD CCharacterModel::GetCurPoseID()
{
	if( _ModelType == MODEL_CHARACTER )
	{
		drPlayPoseInfo* info = LECharacter::GetPlayPoseInfo();
		return info ? info->pose : -1;
	}
	else
	{
		drPlayPoseInfo* info = LECharacter::GetObjImpPlayPoseInfo( 0, ANIM_CTRL_TYPE_MAT );
		return info ? info->pose : -1;
	}
}
...
int CCharacterModel::IsPosePlaying( DWORD pose )
{
	...
	{
		drPlayPoseInfo* info = LECharacter::GetPlayPoseInfo();
		drIPoseCtrl* ctrl = LECharacter::GetPoseCtrl();
	...
	{
		drPlayPoseInfo* info = LECharacter::GetObjImpPlayPoseInfo(0, ANIM_CTRL_TYPE_MAT);
		drIPoseCtrl* ctrl = LECharacter::GetObjImpPoseCtrl(0, ANIM_CTRL_TYPE_MAT);
		if( info == 0 || info->pose != pose )
			return 0;
	...
	{
		drPlayPoseInfo* info = LECharacter::GetObjImpPlayPoseInfo(1, ANIM_CTRL_TYPE_MAT);
		drIPoseCtrl* ctrl = LECharacter::GetObjImpPoseCtrl(1, ANIM_CTRL_TYPE_MAT);
		if( info == 0 || info->pose != pose )
			return 0;
...
DWORD CCharacterModel::GetCurPoseKeyFrameNum()
{
	...
	drPoseInfo* info = c->GetPoseInfo(_SmallPoseID);
	if(info == 0)
		return 0;
...
drIPoseCtrl* CCharacterModel::GetPoseCtrl()
{
	drIPoseCtrl* c = 0;
...
drPoseInfo* CCharacterModel::GetPoseInfo(DWORD pose_id)
{
	drIPoseCtrl* pose_ctrl = GetPoseCtrl();
	if(pose_ctrl == 0)
		return 0;

	return pose_ctrl->GetPoseInfo(pose_id);
}
drPoseInfo* CCharacterModel::GetPoseInfoBig(DWORD big_pose_id)
{
	DWORD p;
...
{
	drIPrimitive* p = GetPhysique()->GetPrimitive(part_id);
	if(p == 0)
		return 0;
...
BOOL CCharacterModel::Cull()
{
	...
	{
		drIPhysique* p = GetPhysique();

		DWORD pri_num = 8;
		for( i = 0; i < pri_num; i++)
		{
			drIPrimitive* pri = p->GetPrimitive(i);
			if(pri == 0)
				continue;
