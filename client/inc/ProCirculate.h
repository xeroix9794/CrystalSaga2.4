...
public:
	...
	// FEATURE: GUILD_BANK
	void UpdateGuildBank(unsigned long chaid, stNetBank* pNetBank);
	void UpdateGuildBankGold(unsigned long chaid, char action, int amount);
	void UpdateGuildPerm(unsigned long chaid, unsigned long permission);
	void UpdateGuildAttr(unsigned char attr);
protected:
    NetIF	*pCNetIf;
