...
{
	char buf[100];
	_snprintf_s(buf,_countof(buf),_TRUNCATE, "Friend:%s says:", name);
...
void CTalkSessionForm::_OnMouseSaveButton(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	{
		string str = "file saved successfully:\"" + CChatRecord::GetLastSavePath() + "\" in";
	...
	else
	{
		...
		CCozeForm::GetInstance()->OnSystemMsg("saving failed ( content is blank, not enough hard disk space or document access is restricted)");
	}
}
...
void CTalkSessionForm::GetMsg(DWORD chaID, string talkMsg)
{
	...
	m_strTalkMsg+=nameBuf+string(" : ")+talkMsg+"\n";
	RefreshForm();
}
...
void CTalkSessionForm::SendBufferMsg()
{
	...
	{
		char *buf = new char[m_strWaitForSend.length()];
	...
	{
		char *buf = new char[m_strWaitForAdded.length()];
