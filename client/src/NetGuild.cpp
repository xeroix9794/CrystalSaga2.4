...
void NetPC_GUILD_PERM(uLong chaID, uLong permission)
{
	if (chaID > 0) {
		CGuildMemberData* pMemberData = CGuildMembersMgr::FindGuildMemberByID(chaID);
...
void NetMC_GUILD_INFO( DWORD dwCharID, DWORD dwGuildID, const char szGuildName[], const char szGuildMotto[], DWORD dwColorId )
{
...
//				LG( pszLogName, "Guild:%u, %s\n", pAll[i]->getAttachID(), pAll[i]->getLogName() );
				...
				pAll[i]->setGuildColor(dwColorId);
			}
		}
	}
	else
