...
DWORD seconds(unsigned long mseconds)
{
...
void CUIGuildList::OnClickApply(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	if (lastApply == 0)
	{
		lastApply = GetTickCount();
		CM_GUILD_TRYFOR(pListData->GetGuildID());
	}
	else
	{
		if (GetTickCount() - lastApply < seconds(15))
		{
			string str = "You must wait 15 seconds before applying to another guild!";
			CBoxMgr::ShowMsgBox(NULL, str.c_str());
		}
		else
		{
			lastApply = GetTickCount();
