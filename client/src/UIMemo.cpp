	...
	while (pszTemp[0])
	{
		...
		if (pszTemp[0] == '<')
		{
			...
			else if (pszTemp[0] == '/')
			{
				...
				const char* pszChrz = ">";
				const char* pszEnd = strstr(pszTemp, pszChrz);
				if (pszEnd == NULL)
				{
					continue;
				}
		...
		// 判断是否一个GBK
BOOL bFlag1 = true;// 0x81 <= (BYTE)pszTemp[0] && (BYTE)pszTemp[0] <= 0xFE;
BOOL bFlag2 = true; //(0x40 <= (BYTE)pszTemp[1] && (BYTE)pszTemp[1] <= 0x7E) || (0x7E <= (BYTE)pszTemp[1] && (BYTE)pszTemp[1] <= 0xFE);
		if (bFlag1 && bFlag2)
		{
			if (sMaxCom - sCom >= 2)
			{
...
void CRichMemo::ParseScript2(const char szInfo[], DWORD dwDefColor, BYTE byFont, USHORT sStartCom, USHORT sMaxCom, BYTE type)
{
	char szData[3];
	memset(szData, 0, 3);
...
/*void CRichMemo::ParseScript( const char szInfo[], DWORD dwDefColor, BYTE byFont, USHORT sStartCom, USHORT sMaxCom )
{
	...
	DWORD dwColor = dwDefColor;
	//const char* pszTemp = szInfo;
	const char* pszTemp = szInfo;// modify by ning.yan 
