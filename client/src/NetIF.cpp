...
BOOL NetIF::HandlePacketMessage(DataSocket *datasock,LPRPACKET pk)
{
	...
	case CMD_MC_UPDATEGUILDBANKGOLD: {
		return SC_UpdateGuildBankGold(pk);
	}
	...
	case CMD_MC_SYNCCHAR: return SC_SYNCCHAR(pk);
	case CMD_MC_GUILD_LISTCHALL: return MC_GUILD_LISTCHALL(pk);
	case CMD_PC_UPDATEGUILDATTR: return PC_GUILDATTR_UPDATE(pk);
	//	Rain. Begin
...
//	case CMD_MC_APPEND_EQUIP: return SC_SynAppendEquip(pk);
	...
	case CMD_MC_GETORIGCOLOR: return SC_GetNameColor(pk);
	break;
...
//·µ»ØÖµ:true-ÔÊÐíÁ¬½Ó,false-²»ÔÊÐíÁ¬½Ó
...
//reasonÖµ:0-±¾µØ³ÌÐòÕý³£ÍË³ö£»-1-Socket´íÎó£»-3-ÍøÂç±»¶Ô·½¹Ø±Õ£»-5-°ü³¤¶È³¬¹ýÏÞÖÆ¡£
...
//------------- Packet·¢ËÍº¯Êý    Client -> Server ÏûÏ¢·¢³ö×Ü¿Ø----------------------------
