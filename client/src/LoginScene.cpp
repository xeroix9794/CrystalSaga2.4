...
CImage*         CLoginScene::imgLogo2    = NULL;
 ...
 int CLoginScene::accountNumber = NULL;
...
void CLoginScene::SetAccount(int accountNumber) {
	if (accountNumber > 0)
	{
		CLoginScene::accountNumber = accountNumber;
	}
	/*
	switch (accountNumber)
	{
	case 1:
		CLoginScene::accountNumber = 1;
		break;
	case 2:
		CLoginScene::accountNumber = 2;
		break;
	case 3:
		CLoginScene::accountNumber = 3;
	case 4:
		CLoginScene::accountNumber = 4;
		break;
	default:
		break;
	}*/
...
void CLoginScene::ShowLoginForm()
{
	...
	m_sPassword = (m_sPassword != "") ? m_sPassword : "";
...
int count_line(istream &is)
{
	...
	is.seekg(0);
	int line_cnt;
	int lf_cnt = std::count(std::istreambuf_iterator<char>(is), std::istreambuf_iterator<char>(), '\n');
	line_cnt = lf_cnt;
...
BOOL CLoginScene::_InitUI()
{
	...
	char tmpFile[128] = { 0 };
#endif
	if (CLoginScene::accountNumber == 0)
		_snprintf_s(tmpFile, _countof(tmpFile), _TRUNCATE, "user\\username.txt");
	else
		_snprintf_s(tmpFile, _countof(tmpFile), _TRUNCATE, "user\\username.txt");

		//_snprintf_s(tmpFile, _countof(tmpFile), _TRUNCATE, "user\\username.txt", CLoginScene::accountNumber);

#if USE_MULTI_ACCOUNT
	ifstream in(tmpFile);
	const int userLines = count_line(in);
	vector<string> usernames(userLines);
#else
	...
	if(in.is_open())
	{
		...
		{
			getline(in, usernames[i]);
		}
		_bAutoInputAct = TRUE;
	}
	
	m_sSaveAccount = string(usernames[CLoginScene::accountNumber]);
	...
	if( edtPassword )
	{
		...
		C3DES des;
		if (CLoginScene::accountNumber == 0)
			_snprintf_s(tmpFile, _countof(tmpFile), _TRUNCATE, "user\\password.txt");
		else
			_snprintf_s(tmpFile, _countof(tmpFile), _TRUNCATE, "user\\password.txt");

			//_snprintf_s(tmpFile, _countof(tmpFile), _TRUNCATE, "user\\password%d.txt", CLoginScene::accountNumber);

		char szPassword[128] = { 0 };
		
#if USE_MULTI_ACCOUNT
		ifstream inPwd(tmpFile);
		const int lines = count_line(inPwd);
		vector<string> passwords(lines);
#else
		ifstream inPwd("user\\password.txt");
#endif
		if (inPwd.is_open())
		{
			int x = 0;
			while (!inPwd.eof()) {
				getline(inPwd, passwords[x]);
				x++;
			}
		}
		m_sPassword = string(passwords[CLoginScene::accountNumber]);
...
void CLoginScene::SavePassword(CCheckBox& chkID, CEdit& edtPassword)
{
	if (!CreateDirectory("user", NULL))
	{
	}
	...
	m_bSaveAccount = chkID.GetIsChecked();
	m_sSaveAccount = string(edtPassword.GetCaption());
	...
	/*
	FILE *fp;
	fopen_s(&fp, "user\\password.txt", "wb");
	if (fp)
	{
		if (m_bSaveAccount)
			fwrite(m_sSaveAccount.c_str(), m_sSaveAccount.size(), 1, fp);
...
void CLoginScene::SaveUserName(CCheckBox& chkID, CEdit& edtID, CEdit& edtPassword)
{
	//创建新文件夹
	if (!CreateDirectory("user", NULL)) 
	{ 
	...
	m_bSaveAccount = chkID.GetIsChecked();
	m_sSaveAccount = string(edtID.GetCaption()).append("\n").append(edtPassword.GetCaption());
