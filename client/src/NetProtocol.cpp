...
//  LG( "select", "NetDelCha - Errno:%d, Info:%s\n", Errno, g_GetServerError(Errno) );
    ...
    if( Errno==0 )
    {
       ...
       // pScene->DelCurrentSelCha();
		//pScene->m_strChar[pScene->m_nCurChaIndex]->SetValid(false);
		//pScene->m_strChar[pScene->m_nCurChaIndex] = NULL;
...
// 		IP("ServerPos Update3(%d, %d)\n", SArea.centre.x, SArea.centre.y);
	...
	if (!pCha->IsNPC() && !pCha->IsMonster())
		pCha->setNameColor(Colors[lNameColor]);
	pCha->SetGuildPermission(lGuildPerm);
	pCha->setGMLv( chGMLv );
	...
	pCha->setGuildColor(lGuildColor);
...
//	LG( p->getLogName(), "MainChaType:%d\n\n", chMainCha );
	...
	{		
		g_stUIChat.GetTeamMgr()->Find( enumTeamRoad )->Add( ulWorldID, szCommName, strMottoName, sIcon );
	}
...
void NetSyncChar(char type, unsigned long cID, unsigned long colorID)
{
	if (CGameApp::GetCurScene()->GetSceneTypeID() == enumWorldScene)
	{
		CCharacter* pCha = CGameApp::GetCurScene()->SearchByID(cID);
		if (pCha)
		{
			if(type == 0)
				pCha->setCharNameColor(Colors[colorID]);
			else if(type == 1)
				pCha->setGuildColor(colorID);
			else
			{
				pCha->setCharNameColor(Colors[colorID]);
				pCha->setGuildColor(colorID);
...
void NetChangeKitbag(DWORD dwChaID, stNetKitbag& SKitbag)
{
	...
	case 3:
		...
		// FEATURE: GUILD_BANK
		grd = g_stUIGuildMgr.GetBankGoodsGrid();
		break;
	case 2:
