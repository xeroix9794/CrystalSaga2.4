...
//	LG( g_LogName.GetLogName( l_id ), RES_STRING(CL_LANGUAGE_MATCH_297), sEmotion );
	return TRUE;
T_E}
bool CanExecuteTimer2(static DWORD& lastClick, int seconds)
{
	if (lastClick == 0)
	{
	...
	{
		if (GetTickCount() - lastClick > (seconds * 1000))
		{
			return true;
		}
		else
...
// 				LG(szLogName, "\n");
		...
		case enumACTION_REQUESTGUILDATTR: {
				...
				for (int i = 1; i < 12; i++)
				{
					int attr = pk.ReadLong();
					attrs[i] = attr;
				}
				g_stUIGuildMgr.AddGuildAttrs(attrs);
				g_stUIGuildMgr.defaultAttr();
				break;
		}
		case enumACTION_REQUESTGUILDLEVEL: {
			long level = pk.ReadLong();

			g_stUIGuildMgr.SetGuildLevel(level);
			break;
		}
...
{
	char type = pk.ReadChar();
	DWORD dwCharID = pk.ReadLong();
	DWORD dwCharColor = pk.ReadLong();
	NetSyncChar(type, dwCharID, dwCharColor);
	return TRUE;
}
...
BOOL SC_SynAppendEquip(LPRPACKET pk)
{T_B
	uLong l_id = pk.ReadLong();
	stNetAppendLook append;
	ReadChaAppendEquip(pk, append);
//	append.ExecEquip(l_id);
	return TRUE;
T_E}
...
{T_B
	...
	SCreateInfo.lGuildPerm = pk.ReadLong();
	...
	else
	{
		...
		for (int i = 0; i < enumEQUIP_NUM; i++)
		{
			...
			{			
				...
				pItem->SetValid(pk.ReadChar() != 0 ? true : true);
				continue;
			}
			else
			{
...
// 	LG(szLogName, "\n");
	//
T_E}
void ReadChaAppendEquip(LPRPACKET pk, stNetAppendLook &append)
{T_B
	for (int i = 0; i < enumEQUIP_NUM; i++) {
		append.sEquipID[i] = pk.ReadShort();
		if (append.sEquipID[i] != 0)
			append.eValid[i] = pk.ReadChar() != 0 ? true : false;
	}
T_E}
...
{T_B
	...
	char p;
	p = pk.ReadChar();
	SNetAppendLook.eValid[0] = (p == 0 ? false : true);

	s = pk.ReadShort();
	SNetAppendLook.sEquipID[1] = s;
	p = pk.ReadChar();
...
BOOL SC_UpdateGuildBankGold(LPRPACKET pk) {

	string gold = pk.ReadString();

	g_stUIGuildMgr.m_plabGuildGold->SetCaption(gold.c_str());
