...
CHeadSay::StateIcons icons();
int CHeadSay::stateCount = NULL;
int CHeadSay::refreshCount = NULL;
vector<int> CHeadSay::allStates;
...
void CHeadSay::Render(D3DXVECTOR3& pos)
{
	...
	if (_IsShowLife)  //血条
	{
		...
		if(_pOwn != pMain->pChaMount && _pOwn->GetShowHP())
			_pImgLife->RenderAll(x - nLifeWidth / 2 - nOffset, y1 + 20, (DWORD)0xFFFFFA9A);
		...
		if (_fManaW > 0) {
			...
			//int nSPWidth = _pImgSP->GetWidth();
			//_pImgSP->SetScaleW(1, _fSPW);

			//_pImgSP->RenderAll(x - nSPWidth / 2 - nOffset, y1 + 30, (DWORD)0xFF00FF);
			CCharacter* pMain = CGameScene::GetMainCha();


			if (precent > 0 && _pOwn->GetShowHPMPText())
			{
				...
				CGuiFont::s_Font.BRender(temp, x - nLifeWidth / 2 - nOffset, y1, (DWORD)color, 0xFF000000);
				_snprintf_s(temp, _countof(temp), _TRUNCATE, "SP : %lld/%lld", _pOwn->getGameAttr()->get(ATTR_SP), _pOwn->getGameAttr()->get(ATTR_MXSP));

				CGuiFont::s_Font.BRender(temp, x - nOffset - (CGuiFont::s_Font.GetWidth(temp) / 2), y1 + 32, spcolour, COLOR_BLACK);
			}
			else if (precent > 0 && _pOwn->IsMonster() && pMain->GetShowHPMPText()) {
				_snprintf_s(temp, _countof(temp), _TRUNCATE, "HP : %lld/%lld (%ld%%)", _pOwn->getGameAttr()->get(ATTR_HP), _pOwn->getGameAttr()->get(ATTR_MXHP), (long)precent);
				CGuiFont::s_Font.BRender(temp, x - nLifeWidth / 2 - nOffset, y1, (DWORD)color, 0xFF000000);
				//_snprintf_s(temp, _countof(temp), _TRUNCATE, "SP : %lld/%lld", _pOwn->getGameAttr()->get(ATTR_SP), _pOwn->getGameAttr()->get(ATTR_MXSP));
...
struct CHeadSay::uiState {
	...
	string GetIcon() { return iconName; }
};

struct CHeadSay::StateIcons {
