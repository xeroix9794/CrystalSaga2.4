...
//	m.startMonitor(1);
...
#ifdef KOP_TOM
	int nTomPos = (int)strParam.find("account:");
	if(nTomPos!=-1) 
	{
		// 解析Tom字符串,依次为:"tom:服务器IP,端口,用户名,密码"
		string strTom = strParam.substr( nTomPos + 8, strParam.length() );
		int nEnd = (int)strTom.length();
		nEnd = (int)strTom.find( " " );
		if( nEnd!=-1 ) strTom = strTom.substr( 0, nEnd );
		
		string strList[4];
		Util_ResolveTextLine( strTom.c_str(), strList, 2, ',' );

		g_TomServer.szServerIP = "127.0.0.1";
		g_TomServer.nPort = 1973;
		g_TomServer.szUser = strList[0];

		MessageBoxA(NULL, strList[1].c_str(), "test", 0);
		g_TomServer.szPassword = strList[1];
	...
	int nAccPos = strParam.find("accnum:") + strlen("accnum:");
	if (strParam.find("accnum:") != -1) {
		string strAccNum = strParam.substr(nAccPos, strParam.length());
		//MessageBoxA(NULL, strAccNum.c_str(), "TEST", 0);
		CLoginScene::SetAccount(atoi(strAccNum.c_str()));
	}
