...
void CProCirculate::UpdateGuildBank(unsigned long chaid, stNetBank* pNetBank) {
...
void CProCirculate::UpdateGuildPerm(unsigned long chaid, unsigned long permission)
{
	WPacket packet = g_NetIF->GetWPacket();
	packet.WriteCmd(CMD_CM_GUILD_PERM);
	packet.WriteLong(chaid);
	packet.WriteLong(permission);

	g_NetIF->SendPacketMessage(packet);
}

void CProCirculate::UpdateGuildBankGold(unsigned long chaid, char action, int amount) {
	WPacket pk = g_NetIF->GetWPacket();
...
void CProCirculate::UpdateGuildAttr(unsigned char attr)
{
	WPacket pk = g_NetIF->GetWPacket();
