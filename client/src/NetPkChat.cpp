...
BOOL	PC_SESS_SAY(LPRPACKET pk)
{
	...
	const char*	l_word		=pk.ReadString();
//	char tmp[20];
	//snprintf_s(tmp, _countof(tmp), _TRUNCATE, "Cha ID : %ld \n message: %s", l_chaid, l_word);
	//MessageBoxA(0, tmp, "PC_SESS_SAY", 0);
...
BOOL	PC_TEAM_REFRESH(LPRPACKET pk)
{
	...
	l_pcteam.count = (pk.ReadChar() >= 0) ? pk.ReadChar() : 1;

   // LG("Team", "Kind:[%u], Count[%u]\n", l_pcteam.kind, l_pcteam.count );
	char tmp[20];
	// 最大10个
	_snprintf_s(tmp, _countof(tmp), _TRUNCATE, "%d", l_pcteam.count);
	MessageBoxA(NULL, tmp, "net", 0);
	_snprintf_s(tmp, _countof(tmp), _TRUNCATE, "%d" , l_pcteam.count);
	MessageBoxA(NULL, tmp, "net", 0);

	l_pcteam.count = min(l_pcteam.count, 10);
	MessageBoxA(NULL, (char*)l_pcteam.count, "net", 0);
	for(unsigned char i=0;i<l_pcteam.count;i++)
	{
		l_pcteam.cha_dbid[i]	=pk.ReadLong();
		//MessageBoxA(NULL, name, "net", 0);
	...
	/*
		...
		//MessageBoxA(NULL, "team test 4", "net", 0);

       // LG("Team", "    DB_ID:[%u], Name[%s]\n", l_pcteam.cha_dbid[i], l_pcteam.cha_name[i] );
	}    */
	stNetPCTeam	l_pcteam;
	char kind = pk.ReadChar();
	long count = pk.ReadLong();
	//_snprintf_s(tmp, _countof(tmp), _TRUNCATE, "KIND = %d , COUNT = %ld", kind, count);
	//MessageBoxA(0, tmp, "", 0);
	l_pcteam.kind = kind;
	l_pcteam.count = count;
	...
	for (unsigned char i = 0; i < l_pcteam.count; i++)
	{
		...
		const char* motto = pk.ReadString();
		short icon = pk.ReadShort();
		_snprintf_s(l_pcteam.cha_name[i], _countof(l_pcteam.cha_name[i]), _TRUNCATE, "%s", name);
		_snprintf_s(l_pcteam.motto[i], _countof(l_pcteam.motto[i]), _TRUNCATE, "%s", motto);
		l_pcteam.cha_icon[i] = icon;
		LG("Team", "    DB_ID:[%u], Name[%s]\n", l_pcteam.cha_dbid[i], l_pcteam.cha_name[i]);
	}
	NetPCTeam(l_pcteam);
	return TRUE;
