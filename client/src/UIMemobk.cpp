	...
	{			
		...
		else if ( s[i] == '_')
		{
			...
			if ( (int)strlen( strTemp.c_str()) > _nMaxNum  )
			{
				...
				{
					int pos = 0;
					std::string tmp = GetSelfString( strTemp, 2*_nMaxNum , true ) ;
					if (tmp[_nMaxNum - 1] >= 0 && tmp[_nMaxNum - 1] <= 127)
					{
						if (tmp[_nMaxNum - 1] != ' ' && tmp[_nMaxNum] != ' ')
						{
							for (int j = 0; j < _nMaxNum; j++)
							{
								if (tmp[_nMaxNum - j - 1] == ' ')
									break;
								pos++;
					...
					_str[_nRowNum] = GetSelfString(strTemp, _nMaxNum - pos, true);
					if (tmp[_nMaxNum - 1] < 0 || tmp[_nMaxNum - 1] > 127)
					{
					...
					else
					{
						...
						{
							strTemp =  strTemp.substr( _str[_nRowNum].length() + 1 , strTemp.length() -_str[_nRowNum].length() - 1 ) ;
						}
					}
					_nRowNum +=1;
				}
				if ((int)strlen(strTemp.c_str()) > 0)
				{
...
,_nLeftMargin(0),_nTopMargin(4),_nRightMargin(4),_nBottomMargin(4),
_nRowHeight(14),_nTitleHeight(24),_nIconHeight(18),_nPhraseNum(0)
{
	_pScroll = new CScroll( frmOwn );	
	_SetSelf();
