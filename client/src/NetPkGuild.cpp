...
BOOL MC_GUILD_INFO(LPRPACKET pk)
{
	...
	DWORD dwGuildColor = pk.ReadLong();
	NetMC_GUILD_INFO( dwCharID, dwGuildID, pszGuildName, pszGuildMotto, dwGuildColor );
	return TRUE;
}
...
BOOL PC_GUILD_PERMISSIONS(LPRPACKET pk)
{
	uLong cha_id = pk.ReadLong();
	uLong permssion = pk.ReadLong();
	NetPC_GUILD_PERM(cha_id, permssion);
	return TRUE;
}
...
BOOL PC_GUILDATTR_UPDATE(LPRPACKET pk)
{
	//l_wpk.WriteLong(guild->m_level);
	//l_wpk.WriteLong(guild->m_exp);
	//l_wpk.WriteLong(guild->m_point);
	int gLevel = pk.ReadLong();
	int gExp = pk.ReadLong();
	int gPoints = pk.ReadLong();
	int gNewAttrValue = pk.ReadLong();
