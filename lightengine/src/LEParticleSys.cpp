...
{
	SAFE_DELETE(_pcPath);
	if (_CPPart && _CPPart->m_iEffNum > 0 && _CPPart->m_iEffNum < 20)
	{
...
//-----------------------------------------------------------------------------
void CLEPartSys::setFontEffectCom(VEC_string& vecText, int num,
...
//-----------------------------------------------------------------------------
void CLEPartSys::setUseZBuff(bool bUseZ)
{
	_bUseZ = bUseZ;
...
//-----------------------------------------------------------------------------
bool CLEPartSys::IsPartArray()
{
	if (_iType == PARTTICLE_MODEL || _iType == PARTTICLE_STRIP || _iType == PARTTICLE_ARRAW)
		return false;
	return true;
}
bool	CLEPartSys::Create(int iType, const s_string& strPartName, int iNumPart,
	...
	CLEResManger	*pCResMagr)
{
	...
	_pTex = pCResMagr->GetTextureByIDdr(id);
	if (_pTex)
	{
...
//-----------------------------------------------------------------------------
void CLEPartSys::SetSkillCtrl(SkillCtrl* pCtrl)
{
	if (!pCtrl)
		return;
...
//-----------------------------------------------------------------------------
void CLEPartSys::SetType(int iType, CLEResManger *pCResMagr)
{
	_iType = iType;
	SetOpertion();
...
//-----------------------------------------------------------------------------
void CLEPartSys::SetFrameCount(WORD	wFrameCount)
{
	Reset(false);
...
void	CLEPartSys::GetRes(int& idtex, int& idmodel, int& ideff)
{
	idtex = -1;
	idmodel = -1;
	ideff = -1;
...
void	CLEPartSys::SetMediayEff(bool bMediay)
{
	if (_bMediay == bMediay)
		return;
	if (bMediay)
...
//-----------------------------------------------------------------------------
void	CLEPartSys::BindingRes(CLEResManger	*pCResMagr)
{
	...
	if (id < 0)
	{
		...
		else
		{
			if (_bMediay)
			{
				_CPPart = new CLEModelEff[_iParNum];
				for (int t = 0; t < _iParNum; t++)
				{
	...
	if (_pCModel)
	{
		...
		{
			//char pszn[128];
			//_snprintf_s(pszn, "texture\\effect\\%s",_strTexName.c_str());
...
void	CLEPartSys::SetTextureName(const s_string& strTexName, CLEResManger	*pCResMagr)
{
	_strTexName = strTexName;
	int id;
	id = pCResMagr->GetTextureID(_strTexName);
...
void	CLEPartSys::SetModelName(const s_string& strModelName, CLEResManger	*pCResMagr)
{
	_strModelName = strModelName;
	//
	//if(_bMediay)
...
//-----------------------------------------------------------------------------
void CLEPartSys::SetEmissionPath(CEffPath* pcPath)
{
	if (!pcPath)
	{
		_bModelRange = false;
...
void  CLEPartSys::SetLoop(bool bLoop)
{
	_bLoop = bLoop;
	if (_CPPart)
	{
...
//-----------------------------------------------------------------------------
void CLEPartSys::Play(int iTime)
{
...
//-----------------------------------------------------------------------------
void CLEPartSys::Stop()
{
	CLEParticle* pParticle;
	switch (_iType)
	{
	case PARTTICLE_SNOW:
	case PARTTICLE_FIRE:
...
//-----------------------------------------------------------------------------
void CLEPartSys::MoveTo(D3DXVECTOR3* vPos, LEMap* pmap)
{
...
//-----------------------------------------------------------------------------
void CLEPartSys::BindingBone(D3DXMATRIX* pMatBone)
{
	if (_bBillBoard)
	{
...
//-----------------------------------------------------------------------------
void CLEPartSys::FrameMove(DWORD dwDailTime)
{
	if (!_bPlay)
		return;
	if (IsDelay())
...
//-----------------------------------------------------------------------------
void	CLEPartSys::RenderVS()
{
}

//-----------------------------------------------------------------------------
void	CLEPartSys::RenderSoft()
{
	//if(_bFontEff)
	//{
...

		{
			for (WORD n = 0; n < _iParNum; ++n)
			{
				if (_vecParticle[n]->m_bLive)
				{
					...
					if (_pCModel->IsItem())
					{
						_pCModel->SetMatrix((drMatrix44*)&_vecParticle[n]->m_SCurMat);
...
//-----------------------------------------------------------------------------
void CLEPartSys::setYaw(float fYaw)
{
	if (_CPPart)
...
//-----------------------------------------------------------------------------
void CLEPartSys::setPitch(float fPitch)
{
	if (_CPPart)
	{
...
//-----------------------------------------------------------------------------
void CLEPartSys::setRoll(float fRoll)
{
	if (_CPPart)
	{
...
//-----------------------------------------------------------------------------
void CLEPartSys::setScale(float fx, float fy, float fz)
{
	_vScale.x = fx;
	_vScale.y = fy;
	_vScale.z = fz;
...
//-----------------------------------------------------------------------------
void CLEPartSys::setDir(float fx, float fy, float fz)
{
	if (!_bModelDir)
		return;
	//float fDist;
...
//-----------------------------------------------------------------------------
void  CLEPartSys::setFontEffect(CLEFont*	pFont)
{
	if (!pFont)
	{
		_bFontEff = false;
...
//-----------------------------------------------------------------------------
void  CLEPartSys::setFontEffText(char* pszText)
{
	_strText = pszText;
}

//-----------------------------------------------------------------------------
void CLEPartSys::Reset(bool	bLife)
{
	_bPlay = false;
	_fCurTime = 0;
	_bPlay = false;
...
bool	_CreateShrink(CLEPartSys* pPart, CLEParticle* pCtrl)
{
	pPart->_fCurTime += *pPart->_pfDailTime;
...
void	_FrameMoveShrink(CLEPartSys* pPart, DWORD	dwDailTime)
{
	WORD wCurFrame;
	WORD wNextFrame;
	float fLerp;
...
bool	CLEPartSys::SaveToFile(FILE* t_pFile)
{
	int tm;
	...
	if (_bModelRange)
	{
		...
		//TODO
		std::vector<D3DXVECTOR3>::pointer pt = &_vecPointRange[0];
		fwrite(pt, sizeof(D3DXVECTOR3), _wVecNum, t_pFile);
	}
...
bool	CLEPartSys::LoadFromFile(FILE* t_pFile, DWORD dwVersion)
{
	int tm;
	...
	fread(t_pszName, sizeof(char), 32, t_pFile);
	//全部转换成小写
	//char *pszDataName = _strdrr( _strdup( t_pszName ) );
	...
	else
	{
		...
		transform(sFileName.begin(), sFileName.end(),
			sFileName.begin(),
			tolower);
		//pszDataName = _strdrr( _strdup( t_pszName ) );
	...
	if (dwVersion > 11)
	{
		if (_bModelRange)
		{
			...
			//TODO
			std::vector<D3DXVECTOR3>::pointer pt = &_vecPointRange[0];
			fread(pt, sizeof(D3DXVECTOR3), _wVecNum, t_pFile);
		}
	}
	if (dwVersion > 12)
	{
...
bool	CLEPartSys::LoadFromMemory(CMemoryBuf*	pbuf, DWORD dwVersion)
{
	int tm;
	...
	if (dwVersion > 11)
	{
		if (_bModelRange)
		{
			...
			//TODO
			std::vector<D3DXVECTOR3>::pointer pt = &_vecPointRange[0];
			pbuf->mread(pt, sizeof(D3DXVECTOR3), _wVecNum);
		}
	}
	if (dwVersion > 12)
	{
...
//-----------------------------------------------------------------------------
void  CLEPartSys::LoadPathFile(char* pszfile)
{
	SAFE_DELETE(_pcPath);
	_pcPath = new CEffPath;
...
//-----------------------------------------------------------------------------
void CLEPartSys::LoadPathFileLet(char* pszfile)
{
	SAFE_DELETE(_pcPath);
	_pcPath = new CEffPath;
...
//-----------------------------------------------------------------------------
void  CLEPartSys::SetPathVel(float fvel)
{
	if (_pcPath)
		_pcPath->SetVel(fvel);
}

float CLEPartSys::GetPathVel()
{
	if (_pcPath)
