...
bool CEffPath::LoadPathFromFileLet(const char* file)
{
    ...
    drMatrix44 mat;
    drEfxTrack et;
    drIAnimDataMatrix* data;
    if(DR_FAILED(et.Load(file)))
        return false;
    data = et.GetData();
    j = data->GetFrameNum();
    for(i = 0; i < j; i++)
...
//!保存效果到文件
bool	CLEModelEff::SaveToFile(char* pszFileName)	
{
	FILE* t_pFile = NULL;
	fopen_s( &t_pFile, pszFileName, "wb");
	if(!t_pFile)
	{
...
//!装入效果从文件
bool	CLEModelEff::LoadFromFile(char* pszFileName)
{
	return true;
}
...
void CLEModelEff::FrameMoveAccel(float fDail)
{

}
void CLEModelEff::Begin()
{
...
void CLEModelEff::RenderAccel(float &fTime)
{
	m_pCEffectFile->SetTechnique(m_iIdxTech);
	m_pCEffectFile->Begin();
	m_pCEffectFile->Pass(0);
	...
	for(int n = 0; n < m_iEffNum; n++)
	{
		...
		if(m_pCEffect->IsItem())
		{
			...
			m_pCEffect->m_pCModel->SetMatrix((drMatrix44*)&m_pCurCortrol->m_SMatResult);
...
void CLEModelEff::FrameMove(DWORD	dwDailTime)
{
...
//#endif
		...
		if(m_pCEffect->IsItem())
		{
			...
			if(!m_pCEffect->IsAlpah())
			{
				...
				//m_pCEffect->m_pDev->SetRenderStateForced(D3DRS_ALPHAFUNC, D3DCLE_GREATER);
			}
			else
			{
				m_pCEffect->m_pDev->SetRenderStateForced(D3DRS_CULLMODE, D3DCULL_CCW);
			...
			if( m_pCEffect->m_pCModel )
			{
				...
				{
					m_pCEffect->m_pCModel->SetMatrix((drMatrix44*)&m_SMatResult);
				}
			...
			{
				//char szData[128];
				//_snprintf_s( szData, "No model file. name = %s.", m_pCEffect->m_strModelName.c_str() );
		...
		//m_pCEffect->m_pDev->SetTextureStageStateForced(0,  D3DTSS_ADDRESSU, D3DTADDRESS_CLALE );
		//m_pCEffect->m_pDev->SetTextureStageStateForced(0,  D3DTSS_ADDRESSV, D3DTADDRESS_CLALE );		
		...
		//Transpose(m_pCurCortrol->m_SMatResult,m_SMatResult);
			...
			if(m_pCEffect->getType() != EFFECT_MODEL )
			{
				...
				m_pCurCortrol->FillTextureUV(m_pCEffect->m_pCModel); //CHECK //CHANGED
...
//#ifdef	MULTITHREAD_LOADING_TEXTURE
		//修改精灵白屏在845上宕机问题 
		if( m_pCEffect->m_CTextruelist.m_pTex==NULL ||
			!m_pCEffect->m_CTextruelist.m_pTex->IsLoadingOK())
			return;
//#endif
...
void	CLEModelEff::ShowCurFrame(int iCurSubEff, int iCurFrame)
{
	for(int n = 0; n < m_iEffNum; n++) //CHANGED: Uncommented //TODO
	{
...
void	CLEModelEff::ShowTempFrame(int iCurSubEff,
...
bool	CLEStrip::SaveToFile(FILE* t_pFile)
{
...
bool	CLEStrip::LoadFromFile(FILE* t_pFile, DWORD dwVersion)
{
...
bool	CLEStrip::LoadFromMemory(CMemoryBuf* pbuf, DWORD dwVersion)
{
...
void	CLEStrip::CopyStrip(CLEStrip* pstrip)
{
