...
#define CLALE(A, MIN, MAX)		( (A) > (MAX) ) ? (MAX) :  ( ( (A) < (MIN) ) ? (MIN) : (A) )


CLEShadeMap::CLEShadeMap(void)
{
...
void	CLEShadeMap::setTextureName(s_string& strName)
{
...
void	CLEShadeMap::BoundingRes(CLEResManger	*m_CResMagr)
{
	int t_iID;
	...
	else
	{
		...
		_lpCurTex = m_CResMagr->GetTextureByIDdr(t_iID);
#else
...
bool	CLEShadeMap::CreateShadeMap(float fRadius)
{
	//if(fRadius > 7.0f)
	//	UseSoft(TRUE);
...
bool	CLEShadeMap::SetGridNum(int iNum)
{
...
void	CLEShadeMap::setFrameTexture(s_string& strTexName, CLEResManger	*pCResMagr)
{
	...
	else
	{
		...
		_lpCurTex = pCResMagr->GetTextureByIDdr(t_iID);
#else
...
void	CLEShadeMap::FrameMove(DWORD	dwDailTime)
{
...
void	CLEShadeMap::MoveTo(D3DXVECTOR3 SVerPos, LEMap* pMap, float	fAngle)
{
	if (!_bShow)
		return;
	if (!_bUpdate)
	...
	/*			if(!TileList ||TileList->IsDefault())
...

			{
				...
				_SShadeUV[iIndex].x = CLALE(fU + 0.5f, 0.0f, 1.0f); //*/_fCos * fU - _fSin * fV   + 0.5f;
				_SShadeUV[iIndex].y = CLALE(fV + 0.5f, 0.0f, 1.0f);//*/_fSin * fU + _fCos * fV   + 0.5f;
...
void	CLEShadeMap::FillVertex()
{
	...
	if (pVertex == 0)
	{
		MessageBox(NULL, "msgLockFailed lock error msglock error CLEShadeMap::FillVertex() line 552", "error", 0);
		//LG("error","msgLockFailed CLEShadeMap::FillVertex() %s\n ", _pModel->m_strName.c_str());
		return;
	}
...
bool	CLEShadeMap::SaveToFile(FILE* pFile)
{
	char t_pszName[32];
...
bool	CLEShadeMap::LoadFromFile(FILE* pFile)
{
	//char t_pszName[32];
...
//class CLEShadeEX
///////////////////////////////////////////////////////////
CLEShadeEX::CLEShadeEX(int iFrameCount)
{
	m_iType = SHADE_ANI;
...
CLEShadeEX::~CLEShadeEX(void)
{
}

bool	CLEShadeEX::CreateSpliteTexture(int iRow, int iColnum)
{
	_iRow = iRow;
	_iCol = iColnum;
...
void	CLEShadeEX::setFrameCount(int iCount)
{
	_iFrameCount = iCount;
}

void	CLEShadeEX::setFrameTime(float fTime)
{
	_fFrameTime = fTime;
}
void	CLEShadeEX::setTexFrameTime(float fTime)
{
	_fTexFrameTime = fTime;
}

void	CLEShadeEX::setFrameColor(int iIdx, D3DCOLOR SColor)
{
	_vecFrameColor[iIdx] = SColor;
}
void	CLEShadeEX::setColor(D3DXCOLOR SColor)
{
	for (int n = 0; n < _iFrameCount; n++)
	{
...
void	CLEShadeEX::FrameMove(DWORD	dwDailTime)
{
	if (!_bShow)
		return;
...
void	CLEShadeEX::MoveTo(D3DXVECTOR3 SVerPos, LEMap* pMap, float	fAngle)
{
	if (!_bShow)
		return;
	if (_SVerPos == SVerPos)
	...
	for (int y = 0; y < _iGridCrossNum + 1; y++)
	{
		for (int x = 0; x < _iGridCrossNum + 1; x++)
		{
			...
			float fU, fV;
			{
				fU = (fGridX - _SVerPos.x) / (_fGridMax * TILESIZE);
				fV = (fGridY - _SVerPos.y) / (_fGridMax * TILESIZE);
				_SShadeUV[iIndex].x = CLALE(fU + 0.5f, 0.0f, 1.0f);
				_SShadeUV[iIndex].y = CLALE(fV + 0.5f, 0.0f, 1.0f);
			}
			if (fAngle != 0)
			{
				D3DXMATRIX mat;
...
bool	CLEShadeEX::SaveToFile(FILE* pFile)
{
	char t_pszName[32];
...
bool	CLEShadeEX::LoadFromFile(FILE* pFile)
{
	//char t_pszName[32];
...
//class CLEShadeCtrl
///////////////////////////////////////////////////////////
CLEShadeCtrl::CLEShadeCtrl(void)
{
	_pShadeMap = NULL;
}
...
bool	CLEShadeCtrl::Create(s_string& strTexName, CLEResManger	*pCResMagr, float fSize,
	bool bAni, int iRow, int iColnum)
{
	SAFE_DELETE(_pShadeMap);
	if (!bAni)
		_pShadeMap = new CLEShadeMap;
	else
		_pShadeMap = new CLEShadeEX;
	...
	if (bAni)
		((CLEShadeEX*)_pShadeMap)->CreateSpliteTexture(iRow, iColnum);

	return true;
}
void	CLEShadeCtrl::SetAlphaType(D3DBLEND eSrcBlend, D3DBLEND eDestBlend)
{
	_pShadeMap->SetAlphaType(eSrcBlend, eDestBlend);
}
/////////////////////
void	CLEShadeCtrl::setFrameCount(int iCount)
{
	if (_pShadeMap->m_iType == SHADE_ANI)
		((CLEShadeEX*)_pShadeMap)->setFrameCount(iCount);
}

void	CLEShadeCtrl::setFrameTime(float fTime)
{
	if (_pShadeMap->m_iType == SHADE_ANI)
		((CLEShadeEX*)_pShadeMap)->setFrameTime(fTime);
}
void	CLEShadeCtrl::setTexFrameTime(float fTime)
{
	if (_pShadeMap->m_iType == SHADE_ANI)
		((CLEShadeEX*)_pShadeMap)->setTexFrameTime(fTime);
}

void	CLEShadeCtrl::setFrameColor(int iIdx, D3DCOLOR SColor)
{
	if (_pShadeMap->m_iType == SHADE_ANI)
		((CLEShadeEX*)_pShadeMap)->setFrameColor(iIdx, SColor);

}
void	CLEShadeCtrl::setColor(D3DXCOLOR SColor)
{
	if (_pShadeMap->m_iType == SHADE_SINGLE)
		_pShadeMap->SetColor(SColor);
	else
		((CLEShadeEX*)_pShadeMap)->SetColor(SColor);
}

int     CLEShadeCtrl::getFrameCount()
{
	if (_pShadeMap->m_iType == SHADE_ANI)
		return ((CLEShadeEX*)_pShadeMap)->getFrameCount();
	return 0;
}

float	CLEShadeCtrl::getFrameTime()
{
	if (_pShadeMap->m_iType == SHADE_ANI)
		return ((CLEShadeEX*)_pShadeMap)->getFrameTime();
	return 0;
}
float	CLEShadeCtrl::getTexFrameTime()
{
	if (_pShadeMap->m_iType == SHADE_ANI)
		return ((CLEShadeEX*)_pShadeMap)->getTexFrameTime();
	return 0;
}
void	CLEShadeCtrl::getFrameColor(int iIdx, D3DCOLOR* pSColor)
{
	if (_pShadeMap->m_iType == SHADE_ANI)
		return ((CLEShadeEX*)_pShadeMap)->getFrameColor(iIdx, pSColor);
}
void	CLEShadeCtrl::getColor(D3DXCOLOR* pSColor)
{
	//if(_pShadeMap->m_iType == SHADE_SINGLE)
	_pShadeMap->getColor(pSColor);
}
/////////////////////
void	CLEShadeCtrl::setFrameTexture(s_string& strTexName, CLEResManger	*pCResMagr)
{
	_pShadeMap->setFrameTexture(strTexName, pCResMagr);
}

void	CLEShadeCtrl::FrameMove(DWORD	dwDailTime)
{
	_pShadeMap->FrameMove(dwDailTime);
}

void	CLEShadeCtrl::MoveTo(D3DXVECTOR3 SVerPos, LEMap* pMap, float	fAngle)
{
	_pShadeMap->MoveTo(SVerPos, pMap, -fAngle);
}
void	CLEShadeCtrl::Render()
{
	_pShadeMap->Render();
}

bool	CLEShadeCtrl::SaveToFile(char* pchName)
{
	FILE* pFile;
	fopen_s(&pFile, pchName, "wb");
	if (!pFile)
		return false;
	...
	if (_pShadeMap->m_iType == SHADE_ANI)
		return ((CLEShadeEX*)_pShadeMap)->SaveToFile(pFile);
	else
...
bool	CLEShadeCtrl::LoadFromFile(char* pchName)
{
	return true;
}
