...
CLEFont::CLEFont() : _hDc(NULL), _hBmp(NULL), _hFont(NULL), _hBmpOld(NULL), _hFontOld(NULL),
...
#ifdef		USE_RENDER
bool CLEFont::CreateFont(LERender* pd3dDevice, char szFontName[], int nSize, int nLevel, DWORD dwFlag)
#else
bool CLEFont::CreateFont(LPDIRECT3DDEVICE8 pd3dDevice, char szFontName[], int nSize, int nLevel, DWORD dwFlag)
#endif
{
	...
	DWORD dwBold = (dwFlag&LEFONT_BOLD) ? FW_BOLD : FW_NORMAL;
	bool dwItalic = (dwFlag&LEFONT_ITALIC) ? TRUE : FALSE;
	bool dwUnLine = (dwFlag&LEFONT_UNLINE) ? TRUE : FALSE;
	...
	drIResourceMgr* res_mgr = _pDev->GetInterfaceMgr()->res_mgr;
	...
	for (int w = 0; w < 4; w++)
	{
		mi.vertex_seq[w] = drVector3(0, 0, 0);
	...
	if (DR_FAILED(_pVB->LoadSystemMemory(&mi)))
		return 0;

	if (DR_FAILED(_pVB->LoadVideoMemory()))
		return 0;
	drTexInfo	info;
	info.stage = 0;
	...
	if (FAILED(drLoadTex(&_pTex, res_mgr, &info)))
	{
		DeleteObject(_hFont);
		DeleteObject(_hBmp);
		DeleteDC(_hDc);
	...
	if (FAILED(drLoadTex(&_pTexFast, res_mgr, &info)))
	{
		DeleteObject(_hFont);
		DeleteObject(_hBmp);
		DeleteDC(_hDc);
...
void CLEFont::BindingRes(CLEResManger* pResMagr)
{
...
int	  CLEFont::GetHzLength(float fscale)
{
	float w = (float)_TextSize * fscale;
	w += ASSIZE;
	return (int)w;
}
int   CLEFont::GetAscLength(float fscale)
{
	float w = (float)_TextSize * fscale;
	w += HLSIZE;
	return (int)w;
...
SIZE* CLEFont::GetTextSize(char* szText, SIZE* pSize, float fScale)
{
	if (!szText)
		return pSize;

	float sx = 0, sy = 0, offset = 0, w = 0, h = 0;
...
bool CLEFont::TextToTexture(char c1, char c2, float & tX, float & tY)
{
	...
	if (it == _vecBuf.end())
	{
		...
		if (it == _vecBuf.end())
		{
			...
			vector<WORD>::pointer pt = &_vecBuf[0];
			memset(pt, 0, sizeof(WORD)*_Max);
...
void CLEFont::DrawTextClipOnce(char* szText, int nLen, LPRECT psrc, LPRECT pclip, D3DXCOLOR color)
{
	if (NULL == _vecAsh || NULL == _vecVer)
	{
		LG("error", "DrawText not Bing Res");
		return;
	}
...
bool CLEFont::DrawTextClip(char* szText, int nLen, LPRECT psrc, LPRECT pclip, D3DXCOLOR color)
{
	int x, y, rcw, rch;
	float sx = 0, sy = 0,
		offset = 0, w = 0, h = 0, tx1 = 0, ty1 = 0, tx2 = 0, ty2 = 0;
...
bool CLEFont::DrawText(int iNumber, int x, int y, D3DXCOLOR color, float fScale)
{
	char psz[64];
	_snprintf_s(psz, 64, _TRUNCATE, "%d", iNumber);
	return DrawText(psz, x, y, color, fScale);
}

bool CLEFont::DrawText(char* szText, int x, int y, D3DXCOLOR color, float fScale, DWORD* dwTime)
{
...
#ifdef DRAWONCE
void  CLEFont::RenderOptimize(int iIdx)
{
	LEFontRect *prc = &_vecFontRect[iIdx];
	_pCEffectFile->SetTechnique(_iRenderIdx);
	_pCEffectFile->Begin();
	_pCEffectFile->Pass(0);
	...
	if (prc->iAshNum)
	{
		...
		std::vector<FONT_VER>::pointer pt = &prc->_vecAsh[0];
		_pDev->GetDevice()->DrawPrimitiveUP(D3DPT_TRIANGLELIST, prc->iAshNum, pt, sizeof(FONT_VER));
	}
	if (prc->iHslNum)
	{
		_pDev->SetTexture(0, _pTex->GetTex());
		...
		std::vector<FONT_VER>::pointer pt = &prc->_vecHsl[0];
		_pDev->GetDevice()->DrawPrimitiveUP(D3DPT_TRIANGLELIST, prc->iHslNum, pt, sizeof(FONT_VER));
	}
	_pCEffectFile->End();
	_pDev->SetRenderStateForced(D3DRS_ZENABLE, TRUE);
	_pDev->SetRenderStateForced(D3DRS_ZWRITEENABLE, TRUE);
}
void CLEFont::RenderDrawOptimize(int iIdx)
{
	LEFontRect *prc = &_vecFontRect[iIdx];
	_pDev->SetVertexShader(D3DFVF_FONT);
...
void CLEFont::BeginClip()
{
}
void CLEFont::EndClip()
{
}

void CLEFont::Draw(char* szText, int x, int y, D3DXCOLOR color)
{
	int nLen = lstrlen(szText);
	if (!szText || nLen == 0)
		return;
...
bool CLEFont::DrawTextShadow(char* szText, int x1, int y1, int x2, int y2,
	D3DXCOLOR color1, D3DXCOLOR color2)
{
	if (!szText)
		return false;
	Begin();
	DrawText(szText, x1, y1, color1, 1.0f); //WAS Draw
	DrawText(szText, x2, y2, color2, 1.0f); //WAS Draw
	End();
	return true;
}

bool CLEFont::Draw3DText(char* szText, D3DXVECTOR3& vPos, D3DXCOLOR color, float fScale)
{
	if (!szText)
		return false;
	if (!_pTex || !_pTex->IsLoadingOK())
	...
	for (int n = 0; n < nLen; n++)
	{
		...
		{
#ifdef USE_RENDER
			drILockableStreamVB*	lpSVB = _pVB->GetLockableStreamVB();
		...
		sx += offset;
#ifdef USE_RENDER
		if (DR_FAILED(_pVB->DrawSubset(0)))
			LG("error", "msgCLEFont draw");
#else
...
bool CLEFont::FindTextFromTex(char c1, char c2, float & tX1, float & tY1, float & tX2, float & tY2)
{
	WORD w = MAKEWORD(c1, c2);
	if (_mapUV.count(w) == 0)
	{
...
void CLEFont::FillTextToTex(char* szText)
{
skip:
