...
BOOL CBlk::Compress_BLK_RLE(const char* filename, DWORD* pARGB, int width, int height, BOOL bAlpha) {
	...
	if (bAlpha) {
		...
		for (i = 0; i < h; i++) {
			for (j = 0; j < w; j++) {
				pAlpha[i*w2 + j] = BYTE(((pARGB[(i + UseRect.top)*width + j + UseRect.left] & 0xff000000) >> 24));
			}
		}
	}
...
BOOL CBlk::CompressPal(const DWORD* pARGB, int width, int height, _RGB* pPal, int & nCount) {
	...
	if (nPalTmpCount > 256) {
		...
		// If the number of colors is greater than 256, take the most used 256
...
int CBlk::GetColorIndex(_RGB* pPal, int nCount, _RGB color) {
	...
	// Locate the index of the specified color in the color index table
