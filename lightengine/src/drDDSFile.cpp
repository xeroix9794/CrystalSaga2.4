...
DR_STD_ILELEMENTATION(drDDSFile)
...
drDDSFile::~drDDSFile()
{
	DR_IF_RELEASE(_origin_tex);
	DR_IF_RELEASE(_dds_tex);
}

DR_RESULT drDDSFile::Clear()
{
	_cubemap_flag = 0;
	_volume_depth = 0;
	_tex_width = 0;
	...
	DR_SAFE_RELEASE(_origin_tex);
	DR_SAFE_RELEASE(_dds_tex);

	return DR_RET_OK;
}

DR_RESULT drDDSFile::LoadOriginTexture(const char* file, DWORD mip_level, D3DFORMAT format, DWORD colorkey)
{
...
DR_RESULT drDDSFile::Compress(D3DFORMAT new_fmt)
{
	DR_RESULT ret = DR_RET_FAILED;
...
HRESULT drDDSFile::BltAllLevels(D3DCUBEMAP_FACES FaceType, IDirect3DBaseTextureX* ptexSrc, IDirect3DBaseTextureX* ptexDest)
{
...
DR_RESULT drDDSFile::Convert(const char* file, D3DFORMAT src_fmt, D3DFORMAT dds_fmt, DWORD mip_level, DWORD src_colorkey)
{
	DR_RESULT ret = DR_RET_FAILED;

	if (DR_FAILED(LoadOriginTexture(file, mip_level, src_fmt, src_colorkey)))
		goto __ret;

	if (DR_FAILED(Compress(dds_fmt)))
		goto __ret;
...
DR_RESULT drDDSFile::Save(const char* file)
{
	DR_RESULT ret = DR_RET_FAILED;
	...
	if (DR_FAILED(SaveDDSHeader(ptex, fp)))
		goto __ret;
...
HRESULT drDDSFile::SaveDDSHeader(IDirect3DBaseTextureX* tex, FILE* fp)
{
	DR_RESULT ret = DR_RET_FAILED;
	...
	if (IsVolumeMap())
	{
		...
		pvoltex->GetLevelDesc(0, &vd);
		fmt = vd.Format;
		size = drGetSurfaceSize(vd.Width, vd.Height, vd.Format);
	...
	else if (IsCubeMap())
	{
		...
		pcubetex->GetLevelDesc(0, &sd);
		fmt = sd.Format;
		size = drGetSurfaceSize(sd.Width, sd.Height, sd.Format);
...
HRESULT drDDSFile::SaveAllMipSurfaces(IDirect3DBaseTextureX* ptex, D3DCUBEMAP_FACES FaceType, FILE* fp)
{
	HRESULT hr;
	...
	for (iLevel = 0; iLevel < _mip_level; iLevel++)
	{
		...
		{
			fwrite(lr.pBits, drGetSurfaceSize(sd.Width, sd.Height, sd.Format), 1, fp);
		}
		else
		{
			DWORD yp;
...
HRESULT drDDSFile::SaveAllVolumeSurfaces(IDirect3DVolumeTextureX* pvoltex, FILE* fp)
{
	HRESULT hr;
	D3DVOLUME_DESC vd;
	D3DBOX box;
