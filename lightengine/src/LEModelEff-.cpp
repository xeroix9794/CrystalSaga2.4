...
bool CLEModelEff::SaveToFile(char* pszFileName) {
	...
	if (!t_pFile) {
		LG("error", "msg %s,Read-only file£¬Open failed", pszFileName);
		return false;
	}
...
void CLEModelEff::FrameMove(DWORD	dwDailTime) {
	...
	for (int n = 0; n < m_iEffNum; n++) {
		...
		// Find out which frame is currently playing
		if (m_pCurCortrol->m_fCurTime >= m_pCEffect->getFrameTime(m_pCurCortrol->m_wCurFrame)) {
			...
			if (m_pCurCortrol->m_wCurFrame >= m_pCEffect->getFrameCount()) {
				...
				if (m_bLoop) {
					...
					// If all the effects are stopped, re-play all the effects
					if (!IsPlay()) {
						Play2(0);
					}
				...
				// Play by number of times
				else  {

					// If all the effects are stopped, stop
					if (!IsPlay()) {
						Stop();
						return;
					}
...
void CLEModelEff::RenderSoft() {
	...
	for (int n = 0; n < m_iEffNum; n++) {
		...
		if (m_pCEffect->m_CTextruelist.m_pTex == NULL || !m_pCEffect->m_CTextruelist.m_pTex->IsLoadingOK()) {
			return;
		}
...
void CLEModelEff::ShowCurFrame(int iCurSubEff, int iCurFrame) {
	{
		...
		D3DXMatrixRotationYawPitchRoll(&t_SMatRot, t_sVerAngle.y, t_sVerAngle.x, t_sVerAngle.z);
...
void CLEModelEff::ShowTempFrame(int iCurSubEff, D3DXVECTOR3& pScale, D3DXVECTOR3& pRotating, D3DXVECTOR3& pTranslate,
												D3DXCOLOR& pColor, TEXCOORD& vecCoord, LPDIRECT3DTEXTURE8 lpTex) {
	m_pCEffect = m_vecEffect[iCurSubEff];

	D3DXMATRIX t_SMat, t_SMatRot;
	D3DXMatrixScaling(&t_SMat, pScale.x, pScale.y, pScale.z);
	D3DXMatrixRotationYawPitchRoll(&t_SMatRot, pRotating.y, pRotating.x, pRotating.z);
	D3DXMatrixMultiply(&m_SMatResult, &t_SMat, &t_SMatRot);
