...
#define MAX_RENDER_TILE		200000 //EDIT_M
#define SEA_TILE_SIZE       4
#define SKYDOOM_TILE_SIZE   6

#define FVF_SKYDOOM          D3DFVF_XYZ|D3DFVF_TEX1 

static DWORD s_dwWaterLoopInitTime = 0;
static int s_nOldWaterLoopFrame = 0;
LEMap::LEMap()
	:_bEdit(FALSE),
	_bSeaVisible(TRUE),
	...
	m_pSkyDoomVB(NULL)
{
	...
	SetShowSize(400, 400); // EDIT_M (36, 30);

	SetSectionBufferSize(16);

#if(defined DRAW_SEA_USE_DYNAMIC_BUFFER)
	_pVertBuf = new LESeaTileVertex[1024];
#endif
...
void LEMap::FrameMove(DWORD dwTimeParam)
{
	if (g_Render.IsRealFPS())
	{
		if (s_dwWaterLoopInitTime == 0)
			s_dwWaterLoopInitTime = g_dwCurFrameTick;

		_nWaterLoopFrame = ((g_dwCurFrameTick - s_dwWaterLoopInitTime) / 30) % MAX_WATER_LOOP;
	}
	else
	{
		_nWaterLoopFrame++;
...
void LEMap::Render()
{
	...
	if (_bUseLandVB)
	{
		if (_pLandVB == NULL)
		{
			...
			_dwLandVBSize = nVertexCnt * sizeof(LETileVertex);
			HRESULT hr = g_Render.GetDevice()->CreateVertexBuffer(_dwLandVBSize,
				0, FVF_LAND, D3DPOOL_MANAGED, &_pLandVB);

			if (FAILED(hr))
			{
				LG("error", "msg LEMap::Render() 创建Land Terrain Vertex Buffer失败!\n");
				_pLandVB = NULL;
			}
		}
	}
	...
	if (_bUseVB)
	{
		...
		if (_pVB == NULL)
		{
			...
			HRESULT hr = g_Render.GetDevice()->CreateVertexBuffer(sizeof(LESeaTileVertex) * nVertexCnt,
				D3DUSAGE_WRITEONLY, FVF_SEA, D3DPOOL_DEFAULT, &_pVB);

			if (FAILED(hr))
			{
				LG("error", "msg LEMap::Render() 创建Sea Vertex Buffer失败!\n");
				_pVB = NULL;
			}
		}
#endif
...
void LEMap::RenderSmMap()
{
	...
	for (y = 0; y < _nShowHeight; y++)
	{
		for (x = 0; x < _nShowWidth; x++)
		{
			...
			for (i = 0; i < 4; i++)
			{
				LETileTex *pLayer = &(pTile->TexLayer[i]);
...
void LEMap::RenderSea()
{
	...
	LESeaTileVertex   *pCurVertex = NULL;
	...
	if (_bUseVB)
	{
		...
		//				for(int i = 0; i < 4; i++)
		//				{
		//					int nCurX = nX + LETile::Offset[i][0] * nSize;
		//					int nCurY = nY + LETile::Offset[i][1] * nSize;
		//				
		//					SVertex[i].p = VECTOR3( (float)nCurX, (float)nCurY, SEA_LEVEL);
		//				
		//					LETile *pTile = GetTile(nCurX, nCurY);
		...
		//				}   
		//				// 0 1 2 
		//				memcpy(&_pVertBuf[num], &SVertex, 3 * sizeof(LESeaTileVertex));
		//				num += 3;  
		//				
		//				// 1 3 4
		...
		//        dsm->BindDataVB(0, _pVertBuf, sizeof(LESeaTileVertex) * num, sizeof(LESeaTileVertex));
		//        if(DR_FAILED(dsm->DrawPrimitive(D3DPT_TRIANGLELIST, 0, num / 3)))
		//            LG("error","msg render sea");
		//
		//#else
		if (s_nOldWaterLoopFrame != _nWaterLoopFrame)
		{
			s_nOldWaterLoopFrame = _nWaterLoopFrame;
			HRESULT hr = _pVB->Lock(0, nVertexCnt * sizeof(LESeaTileVertex), (BYTE**)&pCurVertex, D3DLOCK_NOOVERWRITE);
			if (FAILED(hr)) return;
			for (y = 0; y < nSeaCntY; y++)
			{
				for (x = 0; x < nSeaCntX; x++)
				{
					...
					// 1 2 3 
					memcpy(pCurVertex, &SVertex, 3 * sizeof(LESeaTileVertex));
					pCurVertex += 3;

					// 1 3 4
					memcpy(pCurVertex, &SVertex[3], 1 * sizeof(LESeaTileVertex));
					pCurVertex++;
					memcpy(pCurVertex, &SVertex[2], 1 * sizeof(LESeaTileVertex));
					pCurVertex++;
					memcpy(pCurVertex, &SVertex[1], 1 * sizeof(LESeaTileVertex));
					pCurVertex++;
				}
			}
		...
		g_Render.SetStreamSource(0, _pVB, 0, sizeof(LESeaTileVertex));
	...
	else
	{
		for (y = 0; y < nSeaCntY; y++)
		{
			for (x = 0; x < nSeaCntX; x++)
			{
				...
				drInterfaceMgr* imgr = g_Render.GetInterfaceMgr();
				drIDynamicStreamMgr* dsm = imgr->res_mgr->GetDynamicStreamMgr();
				dsm->BindDataVB(0, &SVertex, sizeof(LESeaTileVertex) * 4, sizeof(LESeaTileVertex));
				if (DR_FAILED(dsm->DrawPrimitive(D3DPT_TRIANGLESTRIP, 0, 2)))
					LG("error", "msg render sea");
#else
				g_Render.GetDevice()->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, &SVertex, sizeof(LESeaTileVertex));
#endif
			}
		}
	}
...
BOOL LEMap::GetPickPos(int nPosX, int nPosY, D3DXVECTOR3 &vPickPos)
{
	...
	for (list<D3DXVECTOR3>::iterator it = PointList.begin(); it != PointList.end(); it++)
	{
		...
		if (pTile)
		{
			for (int i = 0; i < 4; i++)
			{
				LETile *pCurTile = GetTile(nX + LETile::Offset[i][0], nY + LETile::Offset[i][1]);
		...
		for (int i = 0; i < 4; i++)
		{
			//LETile *pCurTile = GetTile(nX + LETile::Offset[i][0], nY + LETile::Offset[i][1]);
			//if(!pCurTile->IsDefault()) fHeight[i] = pCurTile->fHeight;
			fHeight[i] = GetGridHeight((nX + LETile::Offset[i][0]) * 2, \
				(nY + LETile::Offset[i][1]) * 2);
		}
#endif
		float fx1 = (float)nX;
...
BOOL LEMap::GetPickPosEditor(int nPosX, int nPosY, D3DXVECTOR3 &vPickPos)
{
	vPickPos = D3DXVECTOR3(-1, -1, -10000);
...
float LEMap::GetHeight(float fX, float fY)  // ? 难以理解， 还有他的意思？
{
	int nX = (int)fX;
	int nY = (int)fY;
	...
	for (int i = 0; i < 4; i++)
	{
		LETile *pCurTile = GetGroupTile(nX, nY, i);
...
void LEMap::_CalTileNormal(int nX, int nY)
{
...
BOOL LEMap::_AddRenderGroup(BYTE btLayer, int nTexNo, LETile *pTile, short sTileX, short sTileY)
{
...
void LEMap::_GenerateTerrainGroup(int nTileStartX, int nTileStartY)
{
...
void LEMap::_FillVB()
{
	...
	LETileVertex *pCurVertex = NULL;
	HRESULT hr = _pLandVB->Lock(0, nLockVertex * sizeof(LETileVertex), (BYTE**)&pCurVertex, 0);
	if (FAILED(hr))
	{
	...
	for (int j = 0; j < nRenderGroupCnt; j++)
	{
		...
		for (int i = 0; i < pGroup->sTileCnt; i++)
		{
			...
			{
				LETileTex *pLayer = &pTile->TexLayer[nLayer];
			...
			memcpy(pCurVertex, &(LETile::_TVertex[0]), nSize * 3);
			pCurVertex += 3;


			// 1 3 4
			memcpy(pCurVertex, &(LETile::_TVertex[3]), nSize);
			pCurVertex++;
			memcpy(pCurVertex, &(LETile::_TVertex[2]), nSize);
			pCurVertex++;
			memcpy(pCurVertex, &(LETile::_TVertex[1]), nSize);
			pCurVertex++;
...
void LEMap::_RenderVB(BOOL bWireframe)
{
	if (_pLandVB == NULL) return;

	g_Render.GetDevice()->SetStreamSource(0, _pLandVB, sizeof(LETileVertex));
	//g_Render.SetTexture(0, NULL);
	...
	for (int j = 0; j < nRenderGroupCnt; j++)
	{
		...
		// 		LETileVertex *pCurVertex = NULL;
		// 		HRESULT hr = _pLandVB->Lock(nStartVertex* sizeof(LETileVertex), pGroup->sTileCnt * 2 * 3 * sizeof(LETileVertex), (BYTE**)&pCurVertex, D3DLOCK_READONLY);
		// 		if(FAILED(hr)) 
		// 		{
		// 			LG("terrain", "Terrain Land VB Lock Fail!\n");
		// 			return;
		// 		}
		// 		for(int p=0; p<pGroup->sTileCnt*2*3;p++)
		// 		{
		// 			LG("terrain", "(%6.2f,%6.2f,%6.2f) c=%08x uv1(%6.2f,%6.2f) uv1(%6.2f,%6.2f)\n", 
		// 				pCurVertex[p].p.x, 
		// 				pCurVertex[p].p.y, 
		// 				pCurVertex[p].p.z, 
		// 				pCurVertex[p].dwColor, 
		// 				pCurVertex[p].tu1, 	pCurVertex[p].tv1, 
		// 				pCurVertex[p].tu2, 	pCurVertex[p].tv2);
		// 
		// 			if( (p%3)==0 )
		// 				LG("terrain", "\n");
		// 
...
void LEMap::CreateSkyDoom(D3DXVECTOR3 center, float radius, char* txPath, bool hemisphere)
{
	...
	{
		m_pSkyDoomVB = new LESkyDoomVertex[VertNum];
	}

	LESkyDoomVertex* ptVB = m_pSkyDoomVB;
	...
	if (UseShader())
	{
		...
		{
			_snprintf_s(Path, MAX_PATH, _TRUNCATE, "shader\\vs_skydoom.vsh");
		...
		{
			_snprintf_s(Path, MAX_PATH, _TRUNCATE, "shader\\ps_skydoom.psh");
			if (SUCCEEDED(D3DXAssembleShaderFromFile(Path, 0, NULL, &pCode, &pError)))
			{
		...
		drIDynamicStreamMgr* dsm = g_Render.GetInterfaceMgr()->res_mgr->GetDynamicStreamMgr();
		dsm->BindDataVB(0, m_pSkyDoomVB, sizeof(LESkyDoomVertex)*VertNum, sizeof(LESkyDoomVertex));
		if (DR_FAILED(dsm->DrawPrimitive(D3DPT_TRIANGLELIST, 0, VertNum / 3)))
...
void LEMap::SetupPixelFog(DWORD Color, DWORD Mode, float Start, float End, float Density)
{
	// Enable fog blending.
