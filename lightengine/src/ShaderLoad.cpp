...
DR_RESULT LoadShader0(drISysGraphics* sys_graphics)
{
	...
	*/
	...
	for (int i = 0; i < shader_num; i++)
	{
		...
		if (DR_FAILED(shader_mgr->RegisterVertexShader(shader_type[i], path, VS_FILE_HLSL)))
			goto __ret;
	}
	...
	for (int i = 0; i < shader_num; i++)
	{
		...
		{
			drMessageBox("Load Vertex Shader Error");
			return DR_RET_FAILED;
		}
	}

	// drShaderDeclMgr
	drIShaderDeclMgr* decl_mgr = shader_mgr->GetShaderDeclMgr();

	decl_mgr->CreateShaderDeclSet(VDT_PNT0, 8);
	decl_mgr->CreateShaderDeclSet(VDT_PNDT0, 8);

	const DWORD sdci_num = 4;
	drShaderDeclCreateInfo sdci[sdci_num] =
	{
	...
	__ret:
		 ...
		 if (ret != DR_RET_OK)
		 {
			 drMessageBox("LoadShader0 error");
		 }
		 return ret;
...
DR_RESULT LoadShader1(drISysGraphics* sys_graphics)
{
	DR_RESULT ret = DR_RET_FAILED;
...
__ret:
	if (ret != DR_RET_OK)
	{
		drMessageBox("LoadShader1 error");
	}
