...
//-----------------------------------------------------------------------------
HRESULT DXUtil_FindMediaFile( TCHAR* strPath, TCHAR* strFilename, INT PathLen )
{
    HANDLE file;
    ...
    _tcsncpy_s( strPath, strlen( strFilename) + 1, strFilename, _TRUNCATE );
    ...
    // Check if the file exists in the current directory
    _stprintf_s( strPath, PathLen, _T("%s%s"), DXUtil_GetDXSDKMediaPath(), strFilename );
    ...
    _tcsncpy_s( strPath, _tcslen(strFilename ) + 1, strFilename, _TRUNCATE  );
    return E_FAIL;
}
                                 ...
                                 DWORD dwLength, TCHAR* strDefault )
{
    ...
    {
        _tcsncpy_s( strValue, _tcslen( strDefault ) + 1, strDefault, _TRUNCATE  );
    }
...
#else
    if( cchDestChar == -1 )
    	strncpy_s( strDestination, strlen( tstrSource ) + 1, tstrSource, _TRUNCATE );
    else
    	strncpy_s( strDestination, cchDestChar, tstrSource, _TRUNCATE );
#endif
}
...
#ifdef _UNICODE
    if( cchDestChar == -1 )
	    wcsncpy_s( wstrDestination, wcslen( tstrSource ) + 1, tstrSource, _TRUNCATE );
    else
	    wcsncpy_s( wstrDestination, tstrSource, cchDestChar, _TRUNCATE  );
#else
...
#else
    if( cchDestChar == -1 )
    	strncpy_s( tstrDestination, strlen( strSource ) + 1, strSource, _TRUNCATE );
    else
    	strncpy_s( tstrDestination, cchDestChar, strSource, _TRUNCATE );
#endif
}
...
#ifdef _UNICODE
    if( cchDestChar == -1 )
	    wcsncpy( tstrDestination, strlen( wstrSource ) + 1, wstrSource, _TRUNCATE );
    else
...
VOID DXUtil_Trace( TCHAR* strMsg, ... )
{
    ...
    _vsntprintf_s( strBuffer, 512, _TRUNCATE, strMsg, args );
    va_end(args);
