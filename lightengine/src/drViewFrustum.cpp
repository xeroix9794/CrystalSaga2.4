...
DR_STD_ILELEMENTATION(drViewFrustum)

void drViewFrustum::Update(const drMatrix44* mat_viewproj)
{
	drPlane* p;
...
BOOL drViewFrustum::IsPointInFrustum(const drVector3* v)
{
	...
	for (int i = 0; i < 6; i++)
	{
		if (drPlaneClassify(&_plane_seq[i], v) <= 0.0f)
			return 0;
	}
	return 1;
}
BOOL drViewFrustum::IsSphereInFrustum(const drSphere* s)
{
	...
	for (int i = 0; i < 6; i++)
	{
		if (drPlaneClassify(&_plane_seq[i], &s->c) <= -s->r)
			return 0;
	}
	return 1;
}

BOOL drViewFrustum::IsSphereInFrustum(const drSphere* s, DWORD* last_check_plane)
{
	BOOL ret = 0;

	if (last_check_plane && (*last_check_plane) != DR_INVALID_INDEX)
	{
		if (drPlaneClassify(&_plane_seq[*last_check_plane], &s->c) <= -s->r)
			goto __ret;
	}
	...
	if (last_check_plane)
	{
		*last_check_plane = DR_INVALID_INDEX;
	}
...
BOOL drViewFrustum::IsBoxInFrustum(const drBox* b, const drMatrix44* b_mat, DWORD* last_check_plane)
{
	DWORD t;

	if (last_check_plane && (*last_check_plane) != DR_INVALID_INDEX)
	{
		t = drPlaneClassifyBox(&_plane_seq[*last_check_plane], b, b_mat);
	...
	for (DWORD i = 0; i < 6; i++)
	{
		...
		t = drPlaneClassifyBox(&_plane_seq[i], b, b_mat);
...
float drViewFrustum::GetDistanceFromNearPlane(const drVector3* v)
{
	return drPlaneClassify(&_plane_seq[VIEWFRUSTUM_FRONT], v);
}
