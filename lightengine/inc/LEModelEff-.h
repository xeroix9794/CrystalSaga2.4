...
class LIGHTENGINE_API CEffPath
{
public:
	CEffPath() {
		//m_vecPath = NULL;
	...
	{
		//if(m_iFrameCount<=0)
		if (m_iFrameCount <= 0 || m_iCurFrame < 0 || m_iCurFrame > 200) //静态分析修改，边界检查
			return;
		m_bEnd = false;
...
class  LIGHTENGINE_API  CLEModelEff// : public CEffectBase
{
public:
	CLEModelEff(void);
	~CLEModelEff(void);
public:
	...
	void	    BindingRes(CLEResManger* pResMagr)
	{
		m_bPlay = false;
		...
		{
			char szData[128];
			_snprintf_s(szData, 128, _TRUNCATE, "获取特效信息(ID%d)名称失败！", idx);
	...
	void	GetRunningDummyMatrix(D3DXMATRIX* pmat, int idx)
	{
		...
		{
			m_vecEffect[0]->m_pCModel->GetObjDummyRunTimeMatrix((drMatrix44*)pmat, idx);
		}
	}
...
class LIGHTENGINE_API CLEStrip
{
public:
	CLEStrip();
	~CLEStrip();
#ifdef		USE_RENDER
	void	CreateStrip(LERender*	pDev, const s_string& strTexName, CLEResManger* pResMagr)
#else
	void	CreateStrip(LPDIRECT3DDEVICE8	pDev, const s_string& strTexName, CLEResManger* pResMagr)
#endif
	{
		m_pDev = pDev;
	...
	void	BindingRes(CLEResManger* pResMagr)
	{
		...
		else
		{
			...
			_pTex = pResMagr->GetTextureByIDdr(id);
#else
...
public:
	void	AttachCharacter(LECharacter* pCha) { _pCha = pCha; }
	void	AttachItem(LESceneItem* pItem) { _pItem = pItem; }

	void	SetDumy(int iIdx, int iDummy) { _iDummy[iIdx] = iDummy; }
	int 	GetDumy(int iIdx) { return _iDummy[iIdx]; }


	void	GetTrack(drMatrix44* dummy1, drMatrix44* dummy2)
	{
		Strip_Vertex path;
		track tul;
	...
	void				SetTexName(const s_string& str, CLEResManger* pResMagr)
	{
		_strTexName = str;
		int id = pResMagr->GetTextureID(_strTexName);
		if (id < 0)
	...
	void		CopyStrip(CLEStrip* pstrip);
