...
//////////////////////////////////////////////////////////////////////////
//2016修改
class  LIGHTENGINE_API CLEResManger
{
public:
	CLEResManger(void);
	~CLEResManger(void);
...
public:
	bool					LoadTotalVShader(drISysGraphics* sys_graphics);

#ifdef USE_RENDER
	bool					InitRes(LERender*		pDev, D3DXMATRIX* pmat, D3DXMATRIX* pMatviewproj);
#else
	...
	drITex*					GetTextureByIDdr(int iID);
	drITex*					GetTextureByNamedr(const s_string &sName);
	...
	CLEEffectFile*			GetEffectFile() { return &_CEffectFile; }
	...
	//#else
	CLEPartCtrl*			GetPartCtrlByID(int iID);
	//#endif
	...
	CLEPartCtrl*			NewPartCtrl(const s_string& strName);
	void					DeletePartCtrl(int iID);


	void					SendResMessage(const s_string& strPartName, D3DXVECTOR3 vPos, LEMap* pMap);
public:
	//!3D设备
#ifdef		USE_RENDER
	LERender*					m_pDev;
...
protected:
	...
	//#else
	S_BVECTOR<CLEPartCtrl*>				_vecPartCtrl;
	//#endif
	...
	//////////////////////////////////////////////////////////////////////////
	std::vector<CLEPartCtrl*>			_vecPartArray;
...
public:
	/*------------------------------------------------------------
		内存调试才需要这些变量 by Waiting 2009-06-18
	------------------------------------------------------------*/
#ifdef __MEM_DEBUG__
	size_t GetCurUseMemory();
	size_t GetMaxUseMemory();
	DWORD  GetCurObject();
	DWORD  GetMaxObject();
	void   EndMemoryCheck();
#endif
...
inline void	CLEResManger::SendResMessage(const s_string& strPartName, D3DXVECTOR3 vPos, LEMap* pMap)
{
	...
	//#ifndef USE_GAME
	CLEPartCtrl* tctrl = GetPartCtrlByID(id);
	if (!tctrl)
		return;
	WORD idx = *_vecValidID.front();

	_vecPartArray[idx] = new CLEPartCtrl;
...
extern LIGHTENGINE_API CLEResManger     ResMgr;
extern DR_RESULT	g_OnLostDevice();
extern DR_RESULT	g_OnResetDevice();
