...
enum drVertexShaderRegisterInput
{
	VSREG_V_POSITION = 0,
...
enum drVertexDeclarationTypesEnum
{
	// vertex declaration
	__begin_decl_type = 0,
	VDT_PD = 0,
	...
	VDT_INVALID = DR_INVALID_INDEX
};

enum drVertexShaderTypesEnum
{
	...
	VST_INVALID = DR_INVALID_INDEX,

};

enum drVertexShaderLightTypeEnum
{
	VSLT_POINT = 1,
	VSLT_SPOT = 2,
	VSLT_DIRECTIONAL = 3,
	VSLT_INVALID = DR_INVALID_INDEX,
};

enum drVertexShaderAnimTypeEnum
{
	VSAT_NULL = 0x0000,
	...
	VSAT_INVALID = DR_INVALID_INDEX

};

struct drShaderDeclQueryInfo
{
	DWORD decl_type;
	DWORD light_type;
	DWORD anim_type;
...
inline drShaderDeclCreateInfo SDCI_VALUE(DWORD shader, DWORD decl, DWORD light, DWORD anim, const char* file)
{
	drShaderDeclCreateInfo i;
	i.shader_id = shader;
	i.decl_type = decl;
	i.light_type = light;
	i.anim_type = anim;
