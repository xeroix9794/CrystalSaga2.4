...
void CGameScene::SetupVertexFog(drIDeviceObject* dev_obj, float Start, float End, DWORD Color, DWORD Mode, BOOL UseRange, FLOAT Density)
{
    // Enable fog blending.
...
int CGameScene::HitTestSceneObjTerrainForInfluence( D3DXVECTOR3* t_pos, const D3DXVECTOR3* nPos )
{
	...
	for( int i = 0; i < _nSceneObjCnt; i++) 
	{
        ...
        if( SUCCEEDED( ret = pObj->HitTestHelperMesh( &p, (drVector3*)&org, (drVector3*)&ray, "block" ) ) )
        {
            if( s == 0 || h.z < p.pos.z )
            {
                h = p.pos;
        ...
        if( SUCCEEDED( ret = pObj->HitTestHelperMesh( &p, (drVector3*)&org, (drVector3*)&ray, "terrain" ) ) )
        {
            if( s == 0 || h.z < p.pos.z )
            {
                h = p.pos;
...
int CGameScene::HitTestSceneObjTerrain( D3DXVECTOR3* t_pos, const D3DXVECTOR3* pos )
{
    ...
    drVector3 org( *(drVector3*)pos );
    drVector3 ray( 0.0f, 0.0f, -1.0f );
...
int CGameScene::HitTestSceneObjChair( D3DXMATRIX* t_mat, int* h, const D3DXVECTOR3* nOrg, const D3DXVECTOR3* nRay )
{
	...
	for( int i = 0; i < _nSceneObjCnt; i++) 
	{
		...
		if( pObj && pObj->IsValid() )
		{
            drIPrimitive* p = pObj->GetPrimitive( 0 );
            if(p == 0)
                continue;
            ...
            {
                if( SUCCEEDED( pObj->HitTestPrimitive( &u, (drVector3*)nOrg, (drVector3*)nRay ) ) )
                {
                    LEMatrix44Multiply( (drMatrix44*)t_mat, &hbi->mat, p->GetMatrixGlobal() );
            ...
            //    LEMatrix44Multiply( (MPMatrix44*)t_mat, &hbi->mat, p->GetMatrixGlobal() );
		...
		if( pObj->IsValid() )
		{
            ...
            if( hm )
            {
                ...
                if( _tcscmp( hmi->name, "wall" ) == 0 )
                {
                    if( SUCCEEDED( pObj->HitTestPrimitive( &u, (drVector3*)nOrg, (drVector3*)nRay ) ) )
                    {
                        LEMatrix44Multiply( (drMatrix44*)t_mat, &hmi->mat, p->GetMatrixGlobal() );
...
int CGameScene::GetMainChaPickRay(drVector3* org, drVector3* ray)
{
    if(_pMainCha == 0 )
        return 0;
...
#define USE_MAINCHARACTER_POS
    ...
    *(D3DXVECTOR3*)(org) = g_pGameApp->GetMainCam()->m_EyePos;
    ...
    drVector3 target = (drVector3&)GetMainCha()->GetPos();
    *org = target;
    org->z += 100.0f;
#endif
...
BOOL CGameScene::LoadSky(const char* strMapName) {
	if (g_pGameApp->GetSkyBox() && g_pGameApp->GetMainCam()) {
		...
		strcat_s(TopFile, "\\Top.bmp");
		strcat_s(BottomFile, "\\Bottom.bmp");
...
int CGameScene::SetTextureLOD(DWORD level)
{
    ...
    for(int i = start_id; i < id_cnt; i++)
    {
        ...
        if(info)
        {
            drITex* tex = GetTexByID(info->nTextureID);
            if(tex)
            {
                tex->SetLOD(level);
            }
